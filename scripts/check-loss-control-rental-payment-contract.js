...
const rentalGuard=read('routes/rental-loss-prevention.js'),rentalLeaks=read('routes/loss-control-rental-payment-leaks.js'),supplier=read('routes/supplier-payment-loss-prevention.js'),rentals=read('routes/rentals.js'),trace=read('routes/inventory-traceability.js'),server=read('server.js');
for(const [name,src] of [['rental loss prevention',rentalGuard],['rental/payment intelligence',rentalLeaks],['supplier payment prevention',supplier]])new vm.Script(src,{filename:name});
const checks=[
 ...
 ['supplier same-day same-amount patterns are surfaced for review',rentalLeaks.includes("supplier_payment_same_day_same_amount")&&rentalLeaks.includes('duplicate-payment risk')],
 ...
 ['supplier payment prevention also catches same supplier amount and date with different references',supplier.includes("triggers.push('same_supplier_amount_date')")&&supplier.includes('sameDayAmount')],
 ...
 ['supplier similarity override evidence is durable',supplier.includes('supplier_payment_similarity_override_events')&&supplier.includes('prior_payment_ids')],
 ...
 ['rental/payment intelligence requires reports permission',rentalLeaks.includes("router.use(requirePermission('reports'))")]
];
