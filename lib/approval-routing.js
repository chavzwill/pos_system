...
async function listManagerApprovals(employee){
 await ensureApprovalRoutingSchema();
 const {rows}=await db.execute({sql:`SELECT ar.*,d.code department_code,d.name department_name FROM approval_requests ar JOIN departments d ON d.id=ar.department_id WHERE ar.status IN ('submitted','in_review','changes_requested') AND EXISTS (SELECT 1 FROM employee_department_memberships edm WHERE edm.department_id=ar.department_id AND edm.employee_id=? AND edm.active=1 AND edm.is_manager=1 AND (edm.branch_id IS NULL OR edm.branch_id=ar.branch_id)) ORDER BY ar.submitted_at ASC LIMIT 200`,args:[employee.id]});
