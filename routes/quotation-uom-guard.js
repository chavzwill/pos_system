...
async function normalize(req){
  ...
  for(let inputIndex=0;inputIndex<items.length;inputIndex++){
    ...
    evidence.push({inputIndex,productId,enteredQuantity,resolved,baseQuantity,enteredUnitPrice:economics.entered_unit_price,baseUnitPrice:economics.base_unit_price});
  }
...
async function save(req,payload){
  ...
  try{
    for(const e of req.quoteUomEvidence){
      const match=rows[e.inputIndex]||rows.find(x=>Number(x.product_id)===e.productId)||null;
