...
router.post('/refunds/:refundId/settle',requireAnyPermission('pos','reports_financial','wo_assess'),async(req,res)=>{
  try{
    ...
    const method=String(r.refund_method).toLowerCase(),actor=Number(req.employee?.id||req.body?.employee_id)||null,drawerId=Number(req.body?.drawer_session_id)||null,externalRef=String(req.body?.external_refund_reference||'').trim().toUpperCase();
    ...
    try{
      ...
      const tr=await tx.execute({sql:`INSERT INTO transactions(transaction_number,customer_id,employee_id,branch_id,drawer_session_id,subtotal,tax_amount,total,payment_method,amount_tendered,change_amount,notes,source,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[number,r.customer_id,actor,r.branch_id,method==='cash'?drawerId:null,-amount,0,-amount,`service_refund_${method}`,-amount,0,`Service refund ${r.wo_number}; original ${r.original_transaction_number}; ${r.reason}`,'service_refund','refund']});const refundTxId=Number(tr.lastInsertRowid);
