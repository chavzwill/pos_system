...
async function createApprovalRequest(input){
 ...
 if(input.externalRequestId){const existing=await findExternalReplay(sourceSystem,input.externalRequestId,payloadHash);if(existing)return existing;}
 let tx;
 ...
 try{
  ...
  try{await tx.rollback();}catch(rollbackError){console.error('[approval-routing] rollback failed',{internal_error:String(rollbackError?.message||rollbackError)});}
  if(input.externalRequestId){const existing=await waitForExternalReplay(sourceSystem,input.externalRequestId,payloadHash);if(existing)return existing;}
  throw error;
 }
}
...
async function recordApprovalDecision({approvalId,employeeId,expectedVersion,decision,notes,authoritativeResult=null}){
 ...
 try{
  ...
  await tx.commit();
  return r.rows[0];
 }catch(error){try{await tx.rollback();}catch(rollbackError){console.error('[approval-routing] decision rollback failed',{approval_id:approvalId,internal_error:String(rollbackError?.message||rollbackError)});}throw error;}
}
