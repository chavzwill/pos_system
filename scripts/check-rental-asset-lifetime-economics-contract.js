...
const checks=[
 ...
 ['lifetime contribution includes revenue recovery disposal acquisition maintenance allocated consumption and losses',route.includes('revenue+recoveries+disposal-acq-maintenance-allocatedConsumption-otherCosts')],
 ['allocated direct receipts and internal consumption are evidenced separately from requested spend',route.includes('actual_operating_cost')&&route.includes('internal_consumed_cost')&&route.includes('direct_received_cost')&&route.includes('requested_allocated_cost')&&route.includes("source_type IN ('internal_consumption','purchase_receipt')")&&route.includes("source_type='purchase_request'")],
 ['incomplete actual valuation is surfaced rather than invented',route.includes('incomplete_valuation_lines')&&route.includes("valuation_status NOT IN ('fully_valued','actual')")],
 ['economic health is deterministic and threshold-explainable',route.includes('economicHealth')&&route.includes('cost_to_revenue_pct')&&route.includes('thresholds')&&route.includes('observed_utilization_pct')],
 ['repair replacement comparison requires explicit financial evidence',route.includes('/replacement-assessments')&&route.includes('Evidence reference is required for repair/replacement comparison')&&route.includes('repair_estimate')&&route.includes('replacement_cost')],
 ['repair replacement comparison uses trailing evidence and stays advisory',route.includes("datetime('now','-365 days')")&&route.includes("signal")&&route.includes('advisory:true')&&route.includes('No repair, sale, disposal or purchase action is automatic')],
...
require('./check-rental-asset-branch-transfer-contract');
