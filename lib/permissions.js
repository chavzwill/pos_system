...
// A few newer fleet routes originally used the descriptive key
// `rentals_manage`, while the established RBAC tree uses
// `rentals_manage_items`. Treat the former as a compatibility alias so a
// legitimate rental manager is not accidentally locked out of fleet sale or
// buyout controls. This does not broaden authority: it resolves to the same
// existing rental-management permission (or its parent `rentals` permission).
