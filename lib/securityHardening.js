...
const { readCookie, hashToken, loadEmployee, sessionDeadlines } = require('./sessionAuth');
...
const PIN_MAX = 8;
const DIAG_MAX = 30;
const MAX_SECURITY_BUCKETS = 10000;
const MAX_DIAGNOSTIC_BYTES = 64 * 1024;
const MAX_JSON_BYTES = 2 * 1024 * 1024;
const MAX_MULTIPART_BYTES = 25 * 1024 * 1024;
const MAX_OTHER_BODY_BYTES = 5 * 1024 * 1024;
const buckets = new Map();
let permissionHardeningPromise = null;

const PRIVATE_UPLOAD_RULES = [
  { prefix: '/uploads/customer-ids/', permissions: ['customers_sensitive'] },
  { prefix: '/uploads/rental-signatures/', permissions: ['rentals_issue', 'rentals_manage_items'] },
  { prefix: '/uploads/rental-po-attachments/', permissions: ['rentals_checkout', 'rentals_manage_items'] },
  { prefix: '/uploads/po-attachments/', permissions: ['purchasing', 'purchasing_approve', 'purchasing_receive'] },
];

async function ensureSecurityPermissionHardening() {
  if (permissionHardeningPromise) return permissionHardeningPromise;
  permissionHardeningPromise = (async () => {
    await ensureReady();
    const { rows: groups } = await db.execute({ sql: 'SELECT id,name,permissions FROM security_groups', args: [] });
    for (const group of groups) {
      let permissions = {};
      try { permissions = JSON.parse(group.permissions || '{}'); } catch (_) {}
      if (!Object.prototype.hasOwnProperty.call(permissions, 'customers_sensitive')) {
        permissions.customers_sensitive = /^(administrator|manager)$/i.test(String(group.name || '').trim());
        await db.execute({ sql: 'UPDATE security_groups SET permissions=? WHERE id=?', args: [JSON.stringify(permissions), group.id] });
      }
    }
  })().catch(error => {
    permissionHardeningPromise = null;
    throw error;
  });
  return permissionHardeningPromise;
}
...
function pruneBuckets(t = now()) {
  if (buckets.size <= MAX_SECURITY_BUCKETS) return;
  for (const [key, value] of buckets) if (value.resetAt <= t) buckets.delete(key);
  while (buckets.size > MAX_SECURITY_BUCKETS) buckets.delete(buckets.keys().next().value);
}
...
async function authorizePrivateUpload(req, res, rule) {
  try {
    await ensureSecurityPermissionHardening();
    ...
    const { rows: [session] } = await db.execute({
      sql: 'SELECT id,employee_id,created_at,expires_at FROM sessions WHERE token_hash = ? AND revoked_at IS NULL',
      args: [hashToken(raw)],
    });
    if (!session || !sessionDeadlines(session).valid) {
      res.setHeader('Cache-Control', 'private, no-store');
      res.status(401).json({ error: 'Authentication required' });
      return false;
    }
...
function requestBodyLimit(req) {
  ...
  if(path==='/client-diagnostics')return MAX_DIAGNOSTIC_BYTES;
  if(type.includes('application/json')||type.includes('+json'))return MAX_JSON_BYTES;
  if(type.includes('multipart/form-data'))return MAX_MULTIPART_BYTES;
  return MAX_OTHER_BODY_BYTES;
}
function enforceDeclaredBodySize(req,res){
  if(['GET','HEAD','OPTIONS'].includes(req.method))return true;
  const type=String(req.headers['content-type']||'').toLowerCase();
  const expectsJson=type.includes('application/json')||type.includes('+json');
  const raw=req.headers['content-length'];
  if(raw==null||raw===''){
    if(expectsJson){
      res.status(411).json({error:'Content-Length is required for JSON request bodies'});
      return false;
    }
    return true;
  }
  const length=Number(raw),limit=requestBodyLimit(req);
  if(!Number.isFinite(length)||length<0){res.status(400).json({error:'Invalid Content-Length'});return false;}
  if(length>limit){res.setHeader('Connection','close');res.status(413).json({error:'Request body is too large'});return false;}
  return true;
}
...
function securityHeaders(req, res, next) {
  ...
  if(!enforceDeclaredBodySize(req,res))return;
...
module.exports = {
  ...
  ensureSecurityPermissionHardening,
  requestBodyLimit,
