...
function validateMemoryUpload(file,{kind='image'}={}){
  ...
  let detected=detectedType(file.buffer);
  if(detected==='zip'){
    if(ext==='.docx')detected='docx';
    else if(ext==='.xlsx')detected='xlsx';
    else return {ok:false,error:'Unsupported archive upload'};
  }
...
module.exports={IMAGE_TYPES,EVIDENCE_TYPES,detectedType,validateMemoryUpload,imageMulterFilter,evidenceMulterFilter,safeExt};
