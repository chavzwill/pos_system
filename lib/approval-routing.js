...
async function createApprovalRequest(input){
 ...
 if(input.externalRequestId){
  const {rows:[existing]}=await db.execute({sql:'SELECT * FROM approval_requests WHERE source_system=? AND external_request_id=?',args:[input.sourceSystem||'pos',input.externalRequestId]});
  if(existing){if(existing.payload_hash!==payloadHash){const e=new Error('APPROVAL_EXTERNAL_REQUEST_CONFLICT');e.status=409;throw e;}return existing;}
 }
 const tx=await db.transaction('write');
 try{
  const r=await tx.execute({sql:`INSERT INTO approval_requests(request_type,source_system,external_request_id,owning_module,owning_record_id,department_id,branch_id,requester_employee_id,customer_id,organization_ref,member_ref,requested_action,priority,required_permission,status,payload_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,args:[input.requestType,input.sourceSystem||'pos',input.externalRequestId||null,input.owningModule,input.owningRecordId==null?null:String(input.owningRecordId),input.departmentId,input.branchId||null,input.requesterEmployeeId||null,input.customerId||null,input.organizationRef||null,input.memberRef||null,input.requestedAction,input.priority||'normal',input.requiredPermission||null,input.status||'submitted',payloadHash]});
  const row=r.rows[0];
