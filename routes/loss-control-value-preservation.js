...
async function evidenceGaps(days){
  ...
  if(await table('retail_transaction_cost_snapshots')){
    const {rows:[c]}=await db.execute({sql:`SELECT COUNT(DISTINCT s.transaction_id) snapshot_transactions,
      COUNT(DISTINCT CASE WHEN g.complete_lines=g.line_count THEN g.transaction_id END) complete_transactions,
      COUNT(DISTINCT CASE WHEN g.complete_lines<g.line_count THEN g.transaction_id END) incomplete_transactions
      FROM retail_transaction_cost_snapshots s JOIN (
        SELECT transaction_id,COUNT(*) line_count,SUM(CASE WHEN evidence_grade='complete' THEN 1 ELSE 0 END) complete_lines
        FROM retail_transaction_cost_snapshots GROUP BY transaction_id
      ) g ON g.transaction_id=s.transaction_id JOIN transactions t ON t.id=s.transaction_id
      WHERE t.status='completed' AND t.created_at>=datetime('now',?)`,args:[`-${days} days`]});
    const total=Number(c?.snapshot_transactions||0),complete=Number(c?.complete_transactions||0),coverage=total?pct(complete,total):0;
    gaps.push({evidence_area:'retail_sale_time_cost_snapshots',source_records:total,complete_cost_transactions:complete,incomplete_cost_transactions:Number(c?.incomplete_transactions||0),coverage_pct:coverage,blocking:total===0,note:total===0?'No prospective sale-time retail cost snapshots exist in this window yet. Older sales are not assigned today’s mutable catalog cost.':coverage>=95?'Prospective retail cost evidence is substantially complete for transactions captured by the new control.':'Some captured retail transactions still lack complete tracked branch-cost evidence; weaker catalog snapshots remain partial evidence and are not auto-posted as authoritative COGS.'});
  }else gaps.push({evidence_area:'retail_historical_cogs',coverage_pct:0,blocking:true,note:'Sale-time cost snapshots are not available. Full retail gross profit and product profitability are not claimed, and current catalog cost is not used retroactively.'});
  return gaps;
}
