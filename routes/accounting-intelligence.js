...
router.get('/overview', async (req,res) => {
  try {
    const p = period(req);
    const salesArgs=[];
    const salesWhere=wherePeriod('t',p,salesArgs);
    const { rows:[sales] } = await db.execute({ sql:`SELECT
      COUNT(*) transaction_count,
      COALESCE(SUM(t.subtotal),0) subtotal,
      COALESCE(SUM(t.discount_amount),0) discounts,
      COALESCE(SUM(t.tax_amount),0) tax,
      COALESCE(SUM(t.total),0) revenue,
      COALESCE(AVG(t.total),0) avg_ticket,
    ...
    const marginArgs=[];
    const marginWhere=wherePeriod('t',p,marginArgs);
    const { rows:[margin] } = await db.execute({ sql:`SELECT
    ...
    const { rows:[ar] } = await db.execute({ sql:`SELECT
      COALESCE(SUM(CASE WHEN credit_enabled=1 THEN account_balance ELSE 0 END),0) total_ar,
    ...
    const { rows:[aging] } = await db.execute({ sql:`SELECT
      COALESCE(SUM(CASE WHEN age_days<=30 THEN balance_due ELSE 0 END),0) current_30,
      ...
      FROM (
        ...
        FROM transactions t
        WHERE t.status='completed' AND t.payment_method='credit'
      ) x WHERE balance_due>0.001`, args:[] });

    const gp = Number(margin.estimated_gross_profit||0);
    const merchRev = Number(margin.merchandise_revenue||0);
    res.json({ period:p, sales, margin:{...margin, estimated_margin_pct: merchRev ? gp/merchRev*100 : 0, basis:'Estimated using current product cost where a catalog product is linked; not a posted accounting ledger.'}, receivables:{...ar,...aging} });
...
router.get('/exceptions', async (req,res)=>{
  try{
    ...
    const {rows:discounts}=await db.execute({sql:`SELECT t.id,t.transaction_number,t.total,t.subtotal,t.discount_amount,t.created_at,b.name branch_name,
    ...
    const {rows:overdue}=await db.execute({sql:`SELECT c.id,c.customer_number,c.first_name||' '||c.last_name customer_name,c.account_balance,c.credit_limit
    ...
    const {rows:negative}=await db.execute({sql:`SELECT t.id,t.transaction_number,ti.product_id,ti.product_name,ti.quantity,ti.total,COALESCE(pr.cost,0)*ti.quantity est_cost,t.created_at
