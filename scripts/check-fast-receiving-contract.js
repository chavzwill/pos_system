...
check('assistant returns authoritative serial and lot tracking profile',route.includes('getTrackingProfile'));
check('assistant suggests the same primary bin used by stock sync',route.includes('product_bin_assignments')&&route.includes('ORDER BY pba.is_primary DESC')&&route.includes('sb.id bin_id'));
...
check('UI submits only to authoritative purchase-order receive endpoint',ui.includes('`/api/purchase-orders/${poId}/receive`'));
...
check('UI supplies serial identity evidence for serial-tracked stock',ui.includes('serial_numbers')&&ui.includes('serialValues(x)'));
check('UI supplies lot identity evidence for lot-tracked stock',ui.includes('out.lots=')&&ui.includes('lot_number'));
check('tracked identity evidence carries the suggested bin id',ui.includes('x.put_away?.bin_id')&&ui.includes('bin_id:binId'));
