...
async function acquireOne(resourceKey,ttlSeconds){
  ...
  await db.execute({sql:`DELETE FROM business_resource_locks WHERE resource_key=? AND acquired_at < datetime('now','-' || ? || ' seconds')`,args:[resourceKey,ttl]});
...
function withLifecycleLocks(keysFn,{ttlSeconds=120,label='business operation'}={}){
  return async(req,res,next)=>{
    try{
      const requested=normalizeKeys(await keysFn(req));
      if(!requested.length)return next();
      req.__businessLifecycleLocks ||= new Map();
      const needed=requested.filter(key=>!req.__businessLifecycleLocks.has(key));
      if(!needed.length)return next();
      const locks=await acquireMany(needed,ttlSeconds);
      if(!locks){
      ...
      for(const lock of locks)req.__businessLifecycleLocks.set(lock.resourceKey,lock);
      let released=false;
      const release=()=>{
        if(released)return;released=true;
        for(const lock of locks){if(req.__businessLifecycleLocks.get(lock.resourceKey)?.token===lock.token)req.__businessLifecycleLocks.delete(lock.resourceKey);}
        void releaseMany(locks);
      };
