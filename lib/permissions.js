...
function enforceDispatch(req,res){
  ...
  // Department-owned handoffs intentionally continue into their own
  // Purchasing/Sales/Rental/Repair permission middleware.
...
function requirePermission(key) {
  return (req, res, next) => {
    ...
    // A classified Dispatch request authorized by dispatch_* is complete at
    // this layer. Do not re-run the legacy transfers permission immediately
    // afterwards; those checks remain in old routers only for compatibility.
...
module.exports = { PERMISSION_TREE, can, requireAuth, requirePermission, requireAnyPermission, dispatchRequirement, enforceDispatch };