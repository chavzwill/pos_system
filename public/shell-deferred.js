...
function schedule(){const run=()=>{add('/guided-mode.js');add('/guided-mode-orchestrator.js');add('/guided-mode-access.js');add('/total-tools-identity.js');add('/shell-native-support.js');add('/workspace-quality-pass.js');};if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:450});else setTimeout(run,60);}
