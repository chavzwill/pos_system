...
function legacySensitiveMutationGuard(req,res){
  ...
  // accounting-source-sync predates the read/write RBAC split and still has a
  // broad report-reader gate inside its legacy router. Enforce the stronger
  // boundary globally until that router is retired/refactored.
...
async function sessionAuth(req, res, next) {
  ...
  try {
    ...
    // Fail open only at authentication middleware level. Downstream route
    // authorization remains fail-closed because req.employee is absent.
  }
  next();
}

module.exports = { sessionAuth, createSession, destroySession, setSessionCookie, clearSessionCookie, hashToken, loadEmployee, readCookie, legacySensitiveMutationGuard };
