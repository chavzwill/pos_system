...
const { auditApiKeyDenied } = require('./securityAudit');
...
const SCOPE_RULES = [
  ...
  { method: 'GET',  prefix: '/api/catalog-cleanup-dependencies', scope: 'products:read' },
...
function employeeEndpointDenial(path) {
  if (path.startsWith('/api/employee-assist/department-approvals')) {
    return { error: 'API keys cannot operate internal approval workflows.', code: 'APPROVAL_API_KEY_FORBIDDEN' };
  }
  if (path.startsWith('/api/department-approvals')) {
    return { error: 'API keys cannot administer departments.', code: 'DEPARTMENT_API_KEY_FORBIDDEN' };
  }
  return { error: 'API keys are not permitted on this employee endpoint' };
}
...
async function apiKeyAuth(req, res, next) {
  ...
  let keyRow;
  try {
    const { rows } = await db.execute({sql: 'SELECT * FROM api_keys WHERE key_hash = ? AND is_active = 1',args: [hash]});
    keyRow = rows[0];
  } catch (e) {
  ...
  if (!needed) {
    await auditApiKeyDenied(req, {apiKeyId: keyRow.id,apiKeyName: keyRow.name,requiredScope: null,control: 'api_key_endpoint_policy'});
    return res.status(403).json(employeeEndpointDenial(path));
  }
  if (!scopes.includes('*') && !scopes.includes(needed)) {
    await auditApiKeyDenied(req, {apiKeyId: keyRow.id,apiKeyName: keyRow.name,requiredScope: needed,control: 'api_key_scope'});
    return res.status(403).json({ error: `API key missing scope: ${needed}` });
  }

  db.execute({sql: "UPDATE api_keys SET last_used_at = datetime('now') WHERE id = ?",args: [keyRow.id]}).catch(() => {});
  req.apiKey = { id: keyRow.id, name: keyRow.name, scopes, required_scope: needed };
  next();
}

module.exports = { apiKeyAuth, hashKey, SCOPE_RULES, requiredScope, employeeEndpointDenial, VALID_MAX_PER_WINDOW, INVALID_MAX_PER_WINDOW };
