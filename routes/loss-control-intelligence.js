...
async function inventoryShrinkageSignals(p){
  ...
  return rows.map(x=>{const qty=Math.abs(Number(x.variance||0)),loss=r2(qty*Number(x.cost||0)),retail=r2(qty*Number(x.price||0));return {signal_key:signalKey(['cycle_shrink',x.item_id]),signal_type:'cycle_count_shortage',category:'inventory_shrinkage',severity:severityFor(loss),branch_id:x.branch_id,product_id:x.product_id,source_type:'cycle_count_item',source_id:x.item_id,title:`Physical count shortage: ${x.product_name||x.sku||'inventory item'}`,estimated_loss:loss,at_risk_value:Math.max(0,r2(retail-loss)),evidence:{session_id:x.session_id,session_number:x.session_number,book_to_physical_variance:Number(x.variance),missing_quantity:qty,cost_per_unit:Number(x.cost||0),retail_per_unit:Number(x.price||0),branch_name:x.branch_name,detected_at:x.detected_at},recommended_action:'Review serial/lot identity, transfers, sales, adjustments and write-offs since the prior verified count. Treat this as an inventory discrepancy, not an employee misconduct finding.'};});
}
...
async function pricingSignals(p){
  ...
  const {rows}=await db.execute({sql:`SELECT t.id transaction_id,t.transaction_number,t.branch_id,t.employee_id,t.discount_amount,t.subtotal,t.created_at,b.name branch_name,e.first_name||' '||e.last_name employee_name,
      ti.id item_id,ti.product_id,ti.product_name,ti.sku,ti.quantity,ti.total line_gross,COALESCE(ti.discount_amount,0) line_discount,p.cost,
  ...
  const out=[];for(const x of rows){const cost=r2(Number(x.cost||0)*Number(x.quantity||0)),lineDiscount=r2(x.line_discount),orderDiscount=r2(x.allocated_order_discount),net=r2(Number(x.line_gross||0)-lineDiscount-orderDiscount),loss=r2(Math.max(0,cost-net));if(loss<=0.009)continue;out.push({signal_key:signalKey(['below_cost',x.item_id]),signal_type:'below_cost_sale',category:'pricing_margin',severity:severityFor(loss,{high:25000,medium:5000}),branch_id:x.branch_id,employee_id:x.employee_id,product_id:x.product_id,source_type:'transaction_item',source_id:x.item_id,title:`Below-cost sale: ${x.product_name}`,estimated_loss:loss,at_risk_value:0,evidence:{transaction_id:x.transaction_id,transaction_number:x.transaction_number,quantity:Number(x.quantity||0),inventory_cost:cost,line_gross:r2(x.line_gross),line_discount:lineDiscount,allocated_order_discount:orderDiscount,effective_revenue:net,employee_name:x.employee_name,branch_name:x.branch_name,created_at:x.created_at},recommended_action:'Confirm whether this was an approved clearance, warranty/customer-service decision, promotion, data error or margin override. Do not treat the signal alone as misconduct.'});}return out;
}
...
async function returnRiskSignals(p){
  ...
  const {rows}=await db.execute({sql:`SELECT r.employee_id,e.first_name||' '||e.last_name employee_name,r.branch_id,b.name branch_name,COUNT(*) return_count,COALESCE(SUM(a.customer_entitlement_total),0) return_value,
      COALESCE((SELECT SUM(t.total) FROM transactions t WHERE t.employee_id=r.employee_id AND t.status='completed' AND date(t.created_at) BETWEEN date(?) AND date(?)${subBranch}),0) employee_sales
    ...
    WHERE COALESCE(r.status,'completed')!='cancelled' AND date(r.created_at) BETWEEN date(?) AND date(?)${branch} GROUP BY r.employee_id,r.branch_id ORDER BY return_value DESC`,args:[...subArgs,...outerArgs]});
