...
function secureCookieRequired(req) {
  const mode = String(process.env.POS_COOKIE_SECURE || 'auto').trim().toLowerCase();
  if (['1', 'true', 'yes', 'on', 'always'].includes(mode)) return true;
  if (['0', 'false', 'no', 'off', 'never'].includes(mode)) return false;
  // Auto mode follows the actual request transport. This is critical for a
  // POS served over plain HTTP on a trusted local network: NODE_ENV=production
  // must not create a Secure cookie that the browser will refuse to return.
  return Boolean(req?.secure);
}

function setSessionCookie(req, res, rawToken) {
  const secure = secureCookieRequired(req) ? '; Secure' : '';
...
async function loadEmployee(employeeId) {
  ...
  if (!emp) return null;
  emp.permissions = parsePermissions(emp.permissions);
  return emp;
}
