...
router.get('/supplier-performance', async (req,res) => {
  try {
    ...
    for(const r of rows){const sale=saleMap.get(Number(r.supplier_id))||{},ret=returnMap.get(Number(r.supplier_id))||{};r.units_sold=Number(sale.units_sold||0);r.sales_value=Number(sale.sales_value||0);r.estimated_gross_profit=Number(sale.estimated_gross_profit||0);r.units_returned=Number(ret.units_returned||0);r.customer_return_rate=r.units_sold>0?Number((100*r.units_returned/r.units_sold).toFixed(2)):null;r.evidence_status=Number(r.purchase_orders||0)>0&&Number(r.units_ordered||0)>0&&Number(r.units_received||0)>0?'sufficient':'insufficient';r.effective_fulfillment=r.evidence_status==='sufficient'?Number(((Number(r.on_time_rate||0)+Number(r.fill_rate||0)+Number(r.quality_acceptance_rate||0))/3).toFixed(2)):null;if(r.evidence_status!=='sufficient'){r.overall_rating_score=null;r.supplier_rating='Not enough data';continue;}const returnScore=r.customer_return_rate==null?100:Math.max(0,100-Math.min(100,r.customer_return_rate*10));r.overall_rating_score=Number((Number(r.on_time_rate||0)*0.30+Number(r.fill_rate||0)*0.30+Number(r.quality_acceptance_rate||0)*0.25+returnScore*0.15).toFixed(2));r.supplier_rating=r.overall_rating_score>=90?'Excellent':r.overall_rating_score>=80?'Good':r.overall_rating_score>=65?'Needs attention':'Poor';}
    const summary={suppliers:rows.length,suppliers_with_rating_evidence:rows.filter(r=>r.evidence_status==='sufficient').length,sales_value:rows.reduce((a,r)=>a+r.sales_value,0),estimated_gross_profit:rows.reduce((a,r)=>a+r.estimated_gross_profit,0),units_sold:rows.reduce((a,r)=>a+r.units_sold,0),units_returned:rows.reduce((a,r)=>a+r.units_returned,0)};
    res.json({start,end,evidence_window:{start,end},rating_method:'30% on-time delivery + 30% fill rate + 25% received-goods quality + 15% customer-return score; only rated when purchase-order, ordered-unit and received-unit evidence exists.',sales_attribution:'Current product supplier assignment',summary,rows,note:'Sales and returns are attributed using each product current supplier assignment; this is not historical supplier provenance. Estimated gross profit uses the product cost available to the POS and is not a substitute for audited landed-cost profitability.'});
  }catch(e){console.error('Supplier performance report failed',e);res.status(500).json({error:'Unable to load supplier performance right now.',code:'SUPPLIER_PERFORMANCE_REPORT_UNAVAILABLE'});}
});
...
router.get('/supplier-items', async (req,res) => {
  try {
    ...
    const {rows}=await db.execute({sql:`SELECT s.id supplier_id,s.name supplier_name,p.id product_id,p.sku,p.name product_name,p.cost,p.price,
      ...
      WHERE p.active=1 GROUP BY s.id,p.id ORDER BY s.name,sales_value DESC`,args:[start,end,...bf.args]});
    res.json({start,end,sales_attribution:'Current product supplier assignment',summary:{items:rows.length,sales_value:rows.reduce((a,r)=>a+Number(r.sales_value||0),0),estimated_gross_profit:rows.reduce((a,r)=>a+Number(r.estimated_gross_profit||0),0)},rows,note:'Sales are attributed using each product current supplier assignment, not historical supplier provenance. Estimated gross profit uses the product cost available to the POS. Use accounting reports for audited profitability.'});
  }catch(e){console.error('Supplier items report failed',e);res.status(500).json({error:'Unable to load supplier items right now.',code:'SUPPLIER_ITEMS_REPORT_UNAVAILABLE'});}
});
