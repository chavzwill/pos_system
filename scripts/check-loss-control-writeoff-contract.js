...
const checks=[
 ...
 ['theft/destruction requires evidence reference',guard.includes("['theft','destruction']")&&guard.includes('evidence or incident/disposal reference')],
 ['financial authorization evidence is durable',guard.includes('inventory_writeoff_financial_approvals')&&guard.includes('financial_authorizer_employee_id')],
 ['sensitive authorization fields are removed before authoritative approval',guard.includes('delete req.body.writeoff_financial_pin')&&guard.includes('delete req.body.writeoff_financial_reason')],
