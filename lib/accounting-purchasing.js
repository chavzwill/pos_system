...
async function syncPurchasingAccounting({ actorId, stats }){
  ...
  if(await exists('supplier_invoices')){
    ...
    const {rows:invoices}=await db.execute({
      sql:`SELECT id,invoice_number,purchase_order_id,subtotal,tax_amount,total,
           ${optional('freight_amount')},${optional('duty_amount')},${optional('other_landed_cost_amount')},${treatment}
           FROM supplier_invoices WHERE status!='void' ORDER BY id`,args:[]
    });
    for(const inv of invoices){
      const tax=money(inv.tax_amount);
      const tt=String(inv.tax_treatment||'').toLowerCase();
      if(tax>0&&!['recoverable','landed_cost','expense'].includes(tt)){
        stats.evidence_gaps.push({
          supplier_invoice_id:inv.id,invoice_number:inv.invoice_number,type:'supplier_tax_treatment_unclassified',tax_amount:tax,automatic_posting:false,
          reason:'Supplier tax exists but its accounting treatment is not evidenced. Mark it recoverable, landed_cost, or expense before Accounting posts the invoice.'
        });
      }
      ...
      if(landed>0){
        stats.evidence_gaps.push({
          supplier_invoice_id:inv.id,invoice_number:inv.invoice_number,type:'landed_cost_allocation_pending',amount:landed,automatic_inventory_allocation:false,
          reason:'Freight, duty, other landed costs and any tax classified as landed cost are held in Landed Cost Allocation Clearing until they can be allocated to specific received inventory rather than silently distorting product cost.'
        });
      }
      const components=money(money(inv.subtotal)+tax+baseLanded);
      if(Math.abs(components-money(inv.total))>0.01){
        stats.reconciliation_issues.push({supplier_invoice_id:inv.id,invoice_number:inv.invoice_number,type:'supplier_invoice_component_mismatch',component_total:components,recorded_total:money(inv.total)});
      }
    }
    ...
    for(const po of closedPos){
      ...
      if(Number(po.invoice_count||0)>0 && Math.abs(received-invoiced)>0.01){
        stats.reconciliation_issues.push({
          ...
          received_inventory_value:received,invoiced_merchandise_subtotal:invoiced,
          reason:'Only merchandise subtotal is reconciled to receipt value. Supplier tax and landed-cost components are classified separately.'
        });
      }
    }
  }
