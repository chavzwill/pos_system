...
async function getOutstandingQty(executor, productId, branchId) {
  ...
  //
  // Approved missing-asset dispositions increment quantity_returned as an
  // administrative resolution while also recording quantity_missing. The
  // physical unit is removed from branch inventory in the same transaction,
  // so the standard quantity - quantity_returned reservation math remains
  // correct and does not double-reserve a unit that has already been written off.
  let sql = `SELECT COALESCE(SUM(MAX(0,rai.quantity - rai.quantity_returned)),0) as qty
