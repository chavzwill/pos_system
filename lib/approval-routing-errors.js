...
function safeApprovalError(error,fallbackCode='APPROVAL_INTERNAL_ERROR'){
 ...
 const def=DEFINITIONS[key]||DEFINITIONS[fallbackCode]||DEFINITIONS.APPROVAL_INTERNAL_ERROR;
 ...
 return {status:Number(error?.status)||def.status,code:def.code||key||fallbackCode,error:error?.publicMessage||def.message,retryable:false};
}
