...
router.post('/',async(req,res,next)=>{
  try{
    ...
    for(let index=0;index<items.length;index++){
      ...
      if(!Number.isFinite(enteredUnitCost)||enteredUnitCost<0)throw new Error('Purchase unit cost must be a non-negative number');
      ...
      const baseUnitCost=Number((enteredUnitCost/factor).toFixed(6));
      ...
      item.entered_unit_cost=enteredUnitCost;item.base_unit_cost=baseUnitCost;
      ...
      return (async()=>{
        ...
        try{
          ...
          for(const e of req.purchaseUomEvidence){
            ...
            await snapshot(tx,{sourceType:'purchase_order',sourceId:payload.id,sourceLineId:lineId,productId:e.productId,enteredQuantity:e.enteredQuantity,resolved:e.resolved,baseQuantity:e.baseQuantity,enteredUnitPrice:e.enteredUnitCost,baseUnitPrice:e.baseUnitCost});
          }
