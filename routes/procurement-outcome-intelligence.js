...
async function ensureOutcomeIntelligence(){
  ...
  readyPromise=(async()=>{
    await ensureGovernance();await ensurePurchaseReceivingControls();await ensureProcurementFx();
    await db.batch([
...
router.post('/purchase-orders/:poId/actual-charges',requirePermission('purchase_requests'),async(req,res)=>{
  try{const poId=Number(req.params.poId),b=req.body||{},type=String(b.charge_type||'').toLowerCase(),amount=Number(b.amount),ref=String(b.evidence_reference||'').trim(),base=await getBaseCurrency(db),currency=String(b.currency||base).toUpperCase();if(!['freight','duty','brokerage','insurance','handling','other'].includes(type))return res.status(400).json({error:'charge_type must be freight, duty, brokerage, insurance, handling, or other'});if(!(amount>=0)||!ref)return res.status(400).json({error:'Non-negative amount and evidence_reference are required'});if(currency!==base)return res.status(409).json({error:`Actual procurement charges must be recorded in the base currency ${base} until foreign-currency outcome normalization is implemented.`,base_currency:base,submitted_currency:currency});const {rows:[po]}=await db.execute({sql:'SELECT id FROM purchase_orders WHERE id=?',args:[poId]});if(!po)return res.status(404).json({error:'Purchase order not found'});const r=await db.execute({sql:`INSERT INTO procurement_actual_charges(po_id,charge_type,amount,currency,evidence_reference,notes,recorded_by_employee_id) VALUES(?,?,?,?,?,?,?)`,args:[poId,type,amount,base,ref,b.notes||null,req.employee?.id||null]});res.status(201).json({id:Number(r.lastInsertRowid),po_id:poId,currency:base});}catch(e){res.status(500).json({error:e.message});}
});
