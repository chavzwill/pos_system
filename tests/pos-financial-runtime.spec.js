...
test.describe('POS financial runtime certification',()=>{
  test('cash sale, stock restoration, refund settlement and drawer custody remain coherent',async()=>{
    ...
    try{
      // Deterministic test SKU starts at zero, then enters stock through the
      // controlled adjustment path so valuation/cost-pool evidence is created.
      ...
      x=await api(admin.cookie,`/api/products/${product.id}/stock`,{method:'PATCH',body:JSON.stringify({branch_id:branch.id,adjustment:2,reason:'Runtime POS financial certification'})});
      expect(x.status).toBe(200);

      x=await api(admin.cookie,'/api/drawers',{method:'POST',body:JSON.stringify({branch_id:branch.id,name:`Runtime Drawer ${suffix}`})});
      expect(x.status).toBe(201);drawer=x.body;
      x=await api(admin.cookie,'/api/security-groups',{method:'POST',body:JSON.stringify({name:`Runtime Cashier ${suffix}`,description:'Temporary POS financial runtime certification',reason:'Runtime POS financial certification',permissions:{pos:true,transactions_returns:true,transactions_refund:true,drawers_open:true,drawers_close:true}})});
      expect(x.status).toBe(201);group=x.body;
      x=await api(admin.cookie,'/api/employees',{method:'POST',body:JSON.stringify({first_name:'Runtime',last_name:'Cashier',username,password,pin,security_group_id:group.id,default_branch_id:branch.id,must_change_password:false})});
      expect(x.status).toBe(201);employee=x.body;
      const cashier=await login(username,password);

      x=await api(cashier.cookie,'/api/drawers/sessions',{method:'POST',body:JSON.stringify({drawer_id:drawer.id,opening_float:100})});
      expect([200,201]).toContain(x.status);session=x.body;
      ...
      expect(sale.status).toBe(201);expect(sale.body.status).toBe('completed');expect(sale.body.cost_evidence_captured).toBe(true);
      expect(r2(sale.body.total)).toBe(100);
      ...
      const ret=await api(cashier.cookie,`/api/transactions/${transactionId}/return`,{method:'POST',body:JSON.stringify({resolution:'refund',items:[{transaction_item_id:transactionItemId,quantity:1}],notes:'Runtime full return'})});
      expect(ret.status).toBe(201);expect(r2(ret.body.allocation.external_refund_total)).toBe(100);
      ...
      const refund=await api(cashier.cookie,`/api/transactions/returns/${ret.body.id}/settle`,{method:'POST',body:JSON.stringify({drawer_session_id:session.id,tenders:[{method:'cash',amount:100}]})});
      expect(refund.status).toBe(201);expect(r2(refund.body.total)).toBe(100);expect(refund.body.drawer_session_id).toBe(session.id);

      const evidence=await api(cashier.cookie,`/api/drawers/sessions/${session.id}`);expect(evidence.status).toBe(200);
      const cash=evidence.body.net_tenders?.find(t=>t.payment_method==='cash');expect(cash).toBeTruthy();expect(r2(cash.gross_tender)).toBe(100);expect(r2(cash.refunds)).toBe(100);expect(r2(cash.net_movement)).toBe(0);expect(r2(evidence.body.cash_net_movement)).toBe(0);
      ...
      // Preserve transaction/return audit history, but retire temporary master
      // data so the test does not leave operational catalog/users/drawers active.
      ...
      if(group)await api(admin.cookie,`/api/security-groups/${group.id}?reason=Runtime%20POS%20financial%20cleanup`,{method:'DELETE'}).then(()=>{}).catch(()=>{});
