...
function decorate(){const r=guideRoot();if(!r)return;const s=step(),counter=r.querySelector('.tt-guide__step-count');if(counter&&!r.querySelector('.tt-guide__progress')){const m=counter.textContent.match(/of\s+(\d+)/i),total=m?Number(m[1]):0,p=document.createElement('div');p.className='tt-guide__progress';p.setAttribute('aria-hidden','true');p.innerHTML=`<span style="width:${total?Math.min(100,(s/total)*100):0}%"></span>`;counter.insertAdjacentElement('afterend',p);}const card=r.querySelector('.tt-guide__step');if(card&&!card.querySelector('.tt-guide__coach-note')){const n=document.createElement('div');n.className='tt-guide__coach-note';n.textContent=s===1?'I’m opening and verifying the correct workspace now. You do not need to press anything for this setup step.':'Use the highlighted control in the workspace. Guide Me stays open while you work.';card.appendChild(n);}positionCoach();}
...
}catch(err){pause(`Guide Me could not prepare this step. ${err.message}`);}finally{running=false;}}
