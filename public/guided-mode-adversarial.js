...
const root=()=>document.getElementById(GUIDE),q=(s,r=document)=>r?.querySelector?.(s)||null,qa=(s,r=document)=>r?.querySelectorAll?[...r.querySelectorAll(s)]:[];
const norm=s=>String(s||'').trim().replace(/\s+/g,' ');
function task(){const g=root();if(!g)return{title:'',step:'',record:''};return{title:norm(q('.tt-guide__head p',g)?.textContent),step:norm(q('.tt-guide__step-count',g)?.textContent),record:norm(q('.tt-guide__record-context strong,.tt-guide__record-note strong',g)?.textContent)};}
...
function validateContinuity(){raf=0;const g=root();if(!g){baseline=null;return;}const now=snapshot();if(!baseline){baseline=now;return;}const change=meaningfulChange(baseline,now);if(change==='profile'){pause('Your signed-in role or permissions changed while this guide was active. Guide Me paused so the workflow can be revalidated for the current employee.');baseline=now;return;}if(change==='workspace'&&!intendedWorkspaceChange()){pause('The active workspace changed unexpectedly during this guided step. Reopen the required area or choose the task again.');baseline=now;return;}if(change==='workspace'&&intendedWorkspaceChange()){window.TotalToolsGuidedModeIntegrity?.resume?.();}
 baseline=now;}
function schedule(){if(!raf)raf=requestAnimationFrame(validateContinuity);}
function guardRapidLauncher(e){if(!e.target.closest?.('#tt-guide-launcher,#tt-guide-access,#tt-guide-quick'))return;const guides=document.querySelectorAll('#'+GUIDE);if(guides.length>1){[...guides].slice(1).forEach(x=>x.remove());pause('Guide Me recovered from a duplicate launcher event. Only one guide is active.');}}
function revalidateAfterResume(){if(!root())return;if(Date.now()-lastVisibility>30000)pause('You returned after Guide Me was in the background. Recheck the current record and controls before continuing.');schedule();}
function profileWatch(){const sig=profileSig();if(lastProfileSig&&sig&&sig!==lastProfileSig&&root())pause('Your employee permissions changed. Guide Me paused until the current task is revalidated.');if(sig)lastProfileSig=sig;}
...
window.addEventListener('popstate',()=>{if(root()){pause('Browser navigation changed while this workflow was active. Guide Me paused so the current step can be revalidated.');setTimeout(schedule,80);}});
document.addEventListener('visibilitychange',()=>{if(document.hidden){lastVisibility=Date.now();return;}revalidateAfterResume();});
window.addEventListener('pageshow',e=>{if(e.persisted&&root()){pause('This page was restored from browser history. Guide Me paused to prevent stale workflow state.');setTimeout(schedule,80);}});
