...
const checks=[
 ...
 ['reservation availability is checked after existing reservations',lib.includes('const reserved=await getReservedQty')&&lib.includes('available=Math.max(0,physical-reserved)')&&lib.includes('available+1e-9<qty')],
 ...
 ['general availability exposes reserved quantity',status.includes('getActiveReservedQty')&&status.includes('restricted,reserved,available:Math.max(0,onHand-restricted-reserved)')],
 ['disposition cannot consume reserved available stock',status.includes('const state=await getAvailableQty')&&status.includes('state.available+1e-9<qty')&&status.includes('are available for disposition')],
 ['negative manual adjustment protects reserved/restricted stock',adjust.includes('getAvailableQty(tx,productId,branchId)')&&adjust.includes('unrestricted, unreserved stock can be reduced')&&adjust.includes('restricted and')&&adjust.includes('reserved ${normalized.base_uom} are protected')],
