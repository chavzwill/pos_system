...
const rentalGuard=read('routes/rental-loss-prevention.js'),rentalLeaks=read('routes/loss-control-rental-payment-leaks.js'),missingLeaks=read('routes/loss-control-rental-missing-asset-reconciliation.js'),nearLeaks=read('routes/loss-control-supplier-payment-near-duplicates.js'),supplier=read('routes/supplier-payment-loss-prevention.js'),rentals=read('routes/rentals.js'),trace=read('routes/inventory-traceability.js'),server=read('server.js');
for(const [name,src] of [['rental loss prevention',rentalGuard],['rental/payment intelligence',rentalLeaks],['missing rental reconciliation',missingLeaks],['near-duplicate payment intelligence',nearLeaks],['supplier payment prevention',supplier]])new vm.Script(src,{filename:name});
const checks=[
 ...
 ['historical missing-rental reconciliation is mounted under loss control',trace.includes("router.use('/loss-control',require('./loss-control-rental-missing-asset-reconciliation'))")],
 ['historical missing assets marked returned are critical reconciliation gaps',missingLeaks.includes("signal_type:'rental_missing_asset_marked_returned'")&&missingLeaks.includes("severity:'critical'")],
 ['historical missing asset exposure uses replacement or inventory cost without declaring automatic loss',missingLeaks.includes('replacement_value_exposure')&&missingLeaks.includes('inventory_cost_exposure')&&missingLeaks.includes('estimated_loss:0')],
 ['missing-asset scanner recognizes rental accessories and mandatory accessories',missingLeaks.includes('is_accessory')&&missingLeaks.includes('is_mandatory_accessory')],
 ['missing-asset reconciliation performs no autonomous mutation',missingLeaks.includes('No rental, inventory, customer, accounting or employee record was changed automatically')],
 ...
 ['rental/payment intelligence requires reports permission',rentalLeaks.includes("router.use(requirePermission('reports'))")&&missingLeaks.includes("router.use(requirePermission('reports'))")&&nearLeaks.includes("router.use(requirePermission('reports'))")]
];
