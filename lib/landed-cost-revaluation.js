...
async function ensureLandedCostRevaluationSchema(){
  ...
  readyPromise=(async()=>{
    ...
    // The inventory-cost trigger owns immutable layer evidence only. Cost-pool
    // value is changed exclusively by finalizeLandedCostAllocationItem so a
    // normal allocation cannot be counted once by the trigger and again by the
    // historical inventory/COGS replay.
    await db.execute({sql:'DROP TRIGGER IF EXISTS trg_inventory_cost_landed_adjustment',args:[]});
    await db.execute({sql:`CREATE TRIGGER trg_inventory_cost_landed_adjustment
      ...
      BEGIN
        ...
        SELECT 'landed_cost_allocation_item',CAST(NEW.id AS TEXT),l.product_id,l.branch_key,'late_landed_cost_after_consumption',
          ...
          'Landed cost was allocated after tracked inventory consumption began. Historical replay will split the amount between remaining inventory and consumed cost.'
