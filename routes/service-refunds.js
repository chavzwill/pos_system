...
router.post('/refunds/:refundId/settle',requireAnyPermission('pos','reports_financial','wo_assess'),async(req,res)=>{
  try{
    ...
    const method=String(r.refund_method).toLowerCase(),actor=Number(req.body?.employee_id||req.employee?.id)||null,drawerId=Number(req.body?.drawer_session_id)||null,externalRef=String(req.body?.external_refund_reference||'').trim();
    if(method==='cash'){
      if(!actor||!drawerId)return res.status(409).json({error:'Cash refund requires the cashier and an open drawer session.'});
      const {rows:[drawer]}=await db.execute({sql:`SELECT * FROM drawer_sessions WHERE id=? AND status='open'`,args:[drawerId]});if(!drawer||Number(drawer.employee_id)!==actor||Number(drawer.branch_id)!==Number(r.branch_id))return res.status(409).json({error:'Cash refund requires the signed-in cashier’s open drawer at the work-order branch.'});
    }
    ...
    try{
      ...
      const sourceIsDeposit=String(r.original_transaction_number||'')&&Number(r.original_transaction_id)>0;
