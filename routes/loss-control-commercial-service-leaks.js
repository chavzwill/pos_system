...
async function repairLaborSignals(p){
  if(!(await table('work_order_task_time_entries'))||!(await table('work_order_tasks'))||!(await table('work_orders')))return [];
  const minimumMinutes=Math.max(15,await settingNumber('loss_control_repair_unbilled_labor_minutes',30));
  const args=[p.start,p.end];let where='';if(p.branchId){where=' AND wo.branch_id=?';args.push(p.branchId);}
  const {rows}=await db.execute({sql:`SELECT wo.id work_order_id,wo.wo_number,wo.status,wo.branch_id,wo.estimate_labor,b.name branch_name,
      COALESCE(SUM(CASE WHEN te.ended_at IS NOT NULL THEN MAX(0,(julianday(te.ended_at)-julianday(te.started_at))*1440) ELSE 0 END),0) recorded_minutes,
      COUNT(DISTINCT t.id) task_count,COUNT(te.id) time_entry_count
    FROM work_orders wo JOIN work_order_tasks t ON t.work_order_id=wo.id JOIN work_order_task_time_entries te ON te.task_id=t.id
    LEFT JOIN branches b ON b.id=wo.branch_id
    WHERE date(te.started_at) BETWEEN date(?) AND date(?)${where}
    GROUP BY wo.id
    HAVING recorded_minutes>=? AND COALESCE(wo.estimate_labor,0)<=0.01
    ORDER BY recorded_minutes DESC`,args:[...args,minimumMinutes]});
  return rows.map(x=>({signal_key:key(['repair_labor_unbilled',x.work_order_id]),signal_type:'repair_recorded_labor_without_charge',category:'repair_leakage',severity:Number(x.recorded_minutes)>=240?'high':'medium',branch_id:x.branch_id,source_type:'work_order',source_id:x.work_order_id,estimated_loss:0,at_risk_value:0,title:`Recorded repair labor has no labor charge: ${x.wo_number}`,evidence:{work_order_status:x.status,branch_name:x.branch_name,recorded_minutes:Number(Number(x.recorded_minutes||0).toFixed(1)),recorded_hours:Number((Number(x.recorded_minutes||0)/60).toFixed(2)),estimate_labor:r2(x.estimate_labor),task_count:Number(x.task_count||0),time_entry_count:Number(x.time_entry_count||0),review_threshold_minutes:minimumMinutes},recommended_action:'Review technician time against the approved estimate, warranty/goodwill terms and final customer billing. Add an authorized labor charge where appropriate or document why the recorded labor is non-billable. No invoice is changed automatically.'}));
}

async function collect(p){const groups=await Promise.all([promotionSignals(p),repairPartSignals(p),repairLaborSignals(p)]);return groups.flat().sort((a,b)=>Number(b.at_risk_value||0)-Number(a.at_risk_value||0));}
