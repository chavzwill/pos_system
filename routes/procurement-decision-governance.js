...
async function marketSnapshot(executor,requirements){
  const ids=requirements.map(x=>x.product_id);if(!ids.length)return {offers:[],price_breaks:[],candidates:[]};
  ...
  if(offerIds.length){const breakMarks=offerIds.map(()=>'?').join(',');const {rows}=await executor.execute({sql:`SELECT offer_id,min_purchase_qty,unit_cost_per_purchase_uom FROM supplier_price_breaks WHERE offer_id IN (${breakMarks}) ORDER BY offer_id,min_purchase_qty`,args:offerIds});priceBreaks=rows;}
  ...
  return {offers,price_breaks:priceBreaks,candidates};
}
