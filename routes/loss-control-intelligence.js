...
async function columnExists(executor,table,column){if(!(await tableExists(executor,table)))return false;const {rows}=await executor.execute({sql:`PRAGMA table_info(${table})`,args:[]});return rows.some(r=>String(r.name)===String(column));}
...
async function inventoryShrinkageSignals(p){
  ...
  const hasControls=await tableExists(db,'cycle_count_controls');
  const detectedAt=hasControls?'COALESCE(ctl.approved_at,ctl.completed_at,cc.created_at)':'cc.created_at';
  const controlJoin=hasControls?' LEFT JOIN cycle_count_controls ctl ON ctl.session_id=cc.id':'';
  const {rows}=await db.execute({sql:`SELECT cc.id session_id,cc.session_number,cc.branch_id,b.name branch_name,ci.id item_id,ci.product_id,ci.product_name,ci.sku,ci.variance,p.cost,p.price,${detectedAt} detected_at
    FROM cycle_count_sessions cc JOIN cycle_count_items ci ON ci.session_id=cc.id${controlJoin} LEFT JOIN products p ON p.id=ci.product_id LEFT JOIN branches b ON b.id=cc.branch_id
    WHERE cc.status='committed' AND COALESCE(ci.variance,0)<0 AND date(${detectedAt}) BETWEEN date(?) AND date(?)${branch}
    ORDER BY ABS(ci.variance*COALESCE(p.cost,0)) DESC`,args});
  return rows.map(x=>{const qty=Math.abs(Number(x.variance||0)),loss=r2(qty*Number(x.cost||0)),retail=r2(qty*Number(x.price||0));return {signal_key:signalKey(['cycle_shrink',x.item_id]),signal_type:'cycle_count_shortage',category:'inventory_shrinkage',severity:severityFor(loss),branch_id:x.branch_id,product_id:x.product_id,source_type:'cycle_count_item',source_id:x.item_id,title:`Physical count shortage: ${x.product_name||x.sku||'inventory item'}`,estimated_loss:loss,at_risk_value:Math.max(0,r2(retail-loss)),evidence:{session_id:x.session_id,session_number:x.session_number,book_to_physical_variance:Number(x.variance),missing_quantity:qty,cost_per_unit:Number(x.cost||0),retail_per_unit:Number(x.price||0),branch_name:x.branch_name,detected_at:x.detected_at,cost_basis:'Current catalog cost is used for the shortage estimate because cycle-count items do not carry a historical per-unit cost snapshot.'},recommended_action:'Review serial/lot identity, transfers, sales, adjustments and write-offs since the prior verified count. Treat this as an inventory discrepancy, not an employee misconduct finding.'};});
}
async function writeoffSignals(p){
  ...
  return rows.map(w=>{const tracked=Number(w.tracked_value||0)>0,loss=r2(tracked?Number(w.tracked_value):Number(w.quantity||0)*Number(w.cost||0));return {signal_key:signalKey(['writeoff',w.id]),signal_type:'approved_writeoff',category:'inventory_writeoff',severity:severityFor(loss),branch_id:w.branch_id,product_id:w.product_id,source_type:'inventory_writeoff',source_id:w.id,title:`Inventory write-off: ${w.product_name}`,estimated_loss:loss,at_risk_value:0,evidence:{writeoff_number:w.writeoff_number,quantity:Number(w.quantity||0),reason_code:w.reason_code,reason_detail:w.reason_detail,source_status:w.source_status,valuation_status:w.valuation_status,branch_name:w.branch_name,cost_basis:tracked?'Tracked write-off valuation evidence':'Current catalog cost fallback'},recommended_action:w.reason_code==='expiration'||w.reason_code==='obsolescence'?'Review replenishment, demand forecasting and transfer opportunities before future stock reaches the same state.':'Review the operational cause and whether prevention, recovery, warranty, supplier claim or process correction is available.'};});
}
async function pricingSignals(p){
  if(!(await tableExists(db,'transactions'))||!(await tableExists(db,'transaction_items')))return [];
  const hasCostSnapshot=await columnExists(db,'transaction_items','unit_cost_at_sale');
  const costExpr=hasCostSnapshot?'COALESCE(ti.unit_cost_at_sale,p.cost,0)':'COALESCE(p.cost,0)';
  const costBasis=hasCostSnapshot?"CASE WHEN ti.unit_cost_at_sale IS NOT NULL THEN 'sale_cost_snapshot' ELSE 'current_catalog_fallback' END":"'current_catalog_fallback'";
  ...
  const {rows}=await db.execute({sql:`SELECT t.id transaction_id,t.transaction_number,t.branch_id,t.employee_id,t.discount_amount,t.subtotal,t.created_at,b.name branch_name,e.first_name||' '||e.last_name employee_name,
      ti.id item_id,ti.product_id,ti.product_name,ti.sku,ti.quantity,ti.total line_gross,COALESCE(ti.discount_amount,0) line_discount_recorded,${costExpr} unit_cost_evidence,${costBasis} cost_basis,
  ...
  const out=[];for(const x of rows){const cost=r2(Number(x.unit_cost_evidence||0)*Number(x.quantity||0)),orderDiscount=r2(x.allocated_order_discount),net=r2(Number(x.line_gross||0)-orderDiscount),loss=r2(Math.max(0,cost-net));if(loss<=0.009)continue;out.push({signal_key:signalKey(['below_cost',x.item_id]),signal_type:'below_cost_sale',category:'pricing_margin',severity:severityFor(loss,{high:25000,medium:5000}),branch_id:x.branch_id,employee_id:x.employee_id,product_id:x.product_id,source_type:'transaction_item',source_id:x.item_id,title:`Below-cost sale: ${x.product_name}`,estimated_loss:loss,at_risk_value:0,evidence:{transaction_id:x.transaction_id,transaction_number:x.transaction_number,quantity:Number(x.quantity||0),unit_cost_evidence:Number(x.unit_cost_evidence||0),inventory_cost:cost,line_gross:r2(x.line_gross),line_discount_recorded:r2(x.line_discount_recorded),allocated_order_discount:orderDiscount,effective_revenue:net,cost_basis:x.cost_basis,employee_name:x.employee_name,branch_name:x.branch_name,created_at:x.created_at,calculation_note:'Posted transaction-level discount is allocated proportionally across merchandise lines. The transaction engine stores line discount metadata separately but does not currently subtract it from posted sale totals, so it is not deducted again here.'},recommended_action:'Confirm whether this was an approved clearance, warranty/customer-service decision, promotion, data error or margin override. Do not treat the signal alone as misconduct.'});}return out;
}
async function procurementSignals(p){
  if(!(await tableExists(db,'procurement_outcome_snapshots'))||!(await tableExists(db,'procurement_decision_reviews')))return [];
  ...
  return rows.map(o=>({signal_key:signalKey(['procurement_variance',o.review_id]),signal_type:'adverse_procurement_variance',category:'purchasing',severity:severityFor(o.cost_variance),source_type:'procurement_decision_review',source_id:o.review_id,title:`Procurement landed-cost overrun: ${o.decision_key}`,estimated_loss:r2(o.cost_variance),at_risk_value:0,evidence:{outcome_snapshot_id:o.id,review_id:o.review_id,expected_landed_cost:Number(o.expected_landed_cost||0),actual_landed_cost:Number(o.actual_landed_cost||0),variance_pct:Number(o.cost_variance_pct||0),shortage_units:Number(o.shortage_units||0),overage_units:Number(o.overage_units||0),rejected_units:Number(o.rejected_units||0),average_delivery_variance_days:o.average_delivery_variance_days},recommended_action:'Compare the approved supplier recommendation with the PO, receipt, freight/duty evidence and supplier performance to identify the reason for the overrun.'}));
}
async function cashSignals(p){
  ...
  const hasDrawerLink=hasRefunds&&await columnExists(db,'retail_refund_settlements','drawer_session_id');
  const refundMatch=hasDrawerLink?`(rs.drawer_session_id=ds.id OR (rs.drawer_session_id IS NULL AND rs.settled_by_employee_id=ds.employee_id AND COALESCE(rs.branch_id,ds.branch_id)=ds.branch_id AND datetime(rs.settled_at)>=datetime(ds.opened_at) AND datetime(rs.settled_at)<=datetime(COALESCE(ds.closed_at,CURRENT_TIMESTAMP))))`:`rs.settled_by_employee_id=ds.employee_id AND COALESCE(rs.branch_id,ds.branch_id)=ds.branch_id AND datetime(rs.settled_at)>=datetime(ds.opened_at) AND datetime(rs.settled_at)<=datetime(COALESCE(ds.closed_at,CURRENT_TIMESTAMP))`;
  const refundExpr=hasRefunds?`COALESCE((SELECT SUM(rsl.amount) FROM retail_refund_settlement_legs rsl JOIN retail_refund_settlements rs ON rs.id=rsl.settlement_id WHERE rsl.payment_method='cash' AND ${refundMatch}),0)`:'0';
...
async function returnRiskSignals(p){
  ...
  const {rows}=await db.execute({sql:`SELECT r.employee_id,e.first_name||' '||e.last_name employee_name,r.branch_id,b.name branch_name,COUNT(*) return_count,COALESCE(SUM(a.customer_entitlement_total),0) return_value,
      COALESCE((SELECT SUM(t.total) FROM transactions t WHERE t.employee_id=r.employee_id AND t.status='completed' AND (r.branch_id IS NULL OR t.branch_id=r.branch_id) AND date(t.created_at) BETWEEN date(?) AND date(?)),0) employee_sales
    ...
    WHERE COALESCE(r.status,'completed')!='cancelled' AND date(r.created_at) BETWEEN date(?) AND date(?)${outerBranch} GROUP BY r.employee_id,r.branch_id ORDER BY return_value DESC`,args:[p.start,p.end,...outerArgs]});
  const threshold=await settingNumber('loss_control_return_rate_threshold_pct',15),minimum=await settingNumber('loss_control_return_value_threshold',10000);return rows.filter(x=>Number(x.return_value||0)>=minimum&&Number(x.employee_sales||0)>0&&100*Number(x.return_value)/Number(x.employee_sales)>=threshold).map(x=>{const rate=Number((100*Number(x.return_value)/Number(x.employee_sales)).toFixed(2));return {signal_key:signalKey(['return_concentration',x.employee_id,x.branch_id]),signal_type:'return_value_concentration',category:'returns_refunds',severity:rate>=30?'high':rate>=20?'medium':'low',branch_id:x.branch_id,employee_id:x.employee_id,source_type:'employee_return_pattern',source_id:x.employee_id,title:`High return/refund concentration for ${x.employee_name||'employee'}`,estimated_loss:0,at_risk_value:r2(x.return_value),evidence:{period_start:p.start,period_end:p.end,return_count:Number(x.return_count||0),return_value:r2(x.return_value),completed_sales_value:r2(x.employee_sales),return_to_sales_pct:rate,branch_name:x.branch_name},recommended_action:'Review the underlying returns for product/customer patterns, valid service reasons, serial/lot evidence and refund settlement. A high rate is an anomaly signal, not proof of improper behavior.'};});
}
