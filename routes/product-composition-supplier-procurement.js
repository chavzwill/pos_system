...
async function ensureSupplierProcurement(){
  ...
  readyPromise=(async()=>{
    ...
    await db.batch([
      ...
      {sql:'CREATE INDEX IF NOT EXISTS idx_supplier_offer_history_offer ON supplier_offer_history(offer_id,captured_at)'}
    ],'write');
...
function validateOfferInput(b,{partial=false}={}){
  if(!partial&&!(Number(b.supplier_id)>0&&Number(b.product_id)>0&&Number(b.unit_cost_per_purchase_uom)>=0))throw Object.assign(new Error('supplier_id, product_id and non-negative unit_cost_per_purchase_uom are required'),{status:400});
  for(const [name,value] of [['units_per_purchase_uom',b.units_per_purchase_uom],['minimum_order_qty',b.minimum_order_qty],['order_multiple',b.order_multiple]])if(value!=null&&!(Number(value)>0))throw Object.assign(new Error(`${name} must be greater than zero`),{status:400});
  for(const [name,value] of [['unit_cost_per_purchase_uom',b.unit_cost_per_purchase_uom],['freight_per_order',b.freight_per_order],['freight_per_purchase_uom',b.freight_per_purchase_uom],['duty_rate',b.duty_rate],['availability_qty',b.availability_qty]])if(value!=null&&Number(value)<0)throw Object.assign(new Error(`${name} cannot be negative`),{status:400});
  if(b.lead_time_days!=null&&Number(b.lead_time_days)<0)throw Object.assign(new Error('lead_time_days cannot be negative'),{status:400});
  if(b.valid_from&&b.valid_until&&String(b.valid_from)>String(b.valid_until))throw Object.assign(new Error('valid_until cannot be before valid_from'),{status:400});
}
async function captureOfferHistory(executor,offer,employeeId){
  if(!offer)return;
  await executor.execute({sql:`INSERT INTO supplier_offer_history(offer_id,supplier_id,product_id,supplier_sku,purchase_uom,units_per_purchase_uom,unit_cost_per_purchase_uom,currency,minimum_order_qty,order_multiple,lead_time_days,freight_per_order,freight_per_purchase_uom,duty_rate,availability_qty,valid_from,valid_until,active,captured_by_employee_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[offer.id,offer.supplier_id,offer.product_id,offer.supplier_sku,offer.purchase_uom,offer.units_per_purchase_uom,offer.unit_cost_per_purchase_uom,offer.currency,offer.minimum_order_qty,offer.order_multiple,offer.lead_time_days,offer.freight_per_order,offer.freight_per_purchase_uom,offer.duty_rate,offer.availability_qty,offer.valid_from,offer.valid_until,offer.active,employeeId||null]});
}
...
router.post('/offers',requirePermission('suppliers'),async(req,res)=>{try{const b=req.body||{};validateOfferInput(b);const tx=await db.transaction('write');let committed=false;try{const result=await tx.execute({sql:`INSERT INTO supplier_product_offers(supplier_id,product_id,supplier_sku,purchase_uom,units_per_purchase_uom,unit_cost_per_purchase_uom,currency,minimum_order_qty,order_multiple,lead_time_days,freight_per_order,freight_per_purchase_uom,duty_rate,availability_qty,valid_from,valid_until,active,updated_by_employee_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[b.supplier_id,b.product_id,b.supplier_sku||null,b.purchase_uom||'EA',Number(b.units_per_purchase_uom||1),Number(b.unit_cost_per_purchase_uom),b.currency||'JMD',Number(b.minimum_order_qty||1),Number(b.order_multiple||1),b.lead_time_days==null?null:Number(b.lead_time_days),Number(b.freight_per_order||0),Number(b.freight_per_purchase_uom||0),Number(b.duty_rate||0),b.availability_qty==null?null:Number(b.availability_qty),b.valid_from||null,b.valid_until||null,b.active===false?0:1,req.employee?.id||null]});const id=Number(result.lastInsertRowid);for(const br of Array.isArray(b.price_breaks)?b.price_breaks:[])if(Number(br.min_purchase_qty)>0&&Number(br.unit_cost_per_purchase_uom)>=0)await tx.execute({sql:'INSERT INTO supplier_price_breaks(offer_id,min_purchase_qty,unit_cost_per_purchase_uom) VALUES(?,?,?)',args:[id,Number(br.min_purchase_qty),Number(br.unit_cost_per_purchase_uom)]});const {rows:[saved]}=await tx.execute({sql:'SELECT * FROM supplier_product_offers WHERE id=?',args:[id]});await captureOfferHistory(tx,saved,req.employee?.id);await tx.commit();committed=true;res.status(201).json({id});}catch(e){if(!committed)await tx.rollback();throw e;}}catch(e){res.status(e.status||400).json({error:e.message});}});
router.put('/offers/:id',requirePermission('suppliers'),async(req,res)=>{try{const id=Number(req.params.id),b=req.body||{};validateOfferInput(b,{partial:true});const tx=await db.transaction('write');let committed=false;try{const {rows:[old]}=await tx.execute({sql:'SELECT * FROM supplier_product_offers WHERE id=?',args:[id]});if(!old)throw Object.assign(new Error('Supplier offer not found'),{status:404});await captureOfferHistory(tx,old,req.employee?.id);const merged={...old,...b};validateOfferInput(merged);await tx.execute({sql:`UPDATE supplier_product_offers SET supplier_id=?,product_id=?,supplier_sku=?,purchase_uom=?,units_per_purchase_uom=?,unit_cost_per_purchase_uom=?,currency=?,minimum_order_qty=?,order_multiple=?,lead_time_days=?,freight_per_order=?,freight_per_purchase_uom=?,duty_rate=?,availability_qty=?,valid_from=?,valid_until=?,active=?,updated_by_employee_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,args:[merged.supplier_id,merged.product_id,merged.supplier_sku||null,merged.purchase_uom||'EA',Number(merged.units_per_purchase_uom||1),Number(merged.unit_cost_per_purchase_uom),merged.currency||'JMD',Number(merged.minimum_order_qty||1),Number(merged.order_multiple||1),merged.lead_time_days==null?null:Number(merged.lead_time_days),Number(merged.freight_per_order||0),Number(merged.freight_per_purchase_uom||0),Number(merged.duty_rate||0),merged.availability_qty==null?null:Number(merged.availability_qty),merged.valid_from||null,merged.valid_until||null,merged.active===false||Number(merged.active)===0?0:1,req.employee?.id||null,id]});if(Array.isArray(b.price_breaks)){await tx.execute({sql:'DELETE FROM supplier_price_breaks WHERE offer_id=?',args:[id]});for(const br of b.price_breaks)if(Number(br.min_purchase_qty)>0&&Number(br.unit_cost_per_purchase_uom)>=0)await tx.execute({sql:'INSERT INTO supplier_price_breaks(offer_id,min_purchase_qty,unit_cost_per_purchase_uom) VALUES(?,?,?)',args:[id,Number(br.min_purchase_qty),Number(br.unit_cost_per_purchase_uom)]});}const {rows:[saved]}=await tx.execute({sql:'SELECT * FROM supplier_product_offers WHERE id=?',args:[id]});await captureOfferHistory(tx,saved,req.employee?.id);await tx.commit();committed=true;res.json(saved);}catch(e){if(!committed)await tx.rollback();throw e;}}catch(e){res.status(e.status||400).json({error:e.message});}});
...
module.exports.offersForProduct=offersForProduct;
module.exports.requirementPlan=requirementPlan;
module.exports.kitCandidates=kitCandidates;
