...
router.post('/hold', requirePermission('pos_hold'), async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      subtotal += lineTotal; tax_amount += lineTax;
    }
    subtotal = parseFloat(subtotal.toFixed(2)); tax_amount = parseFloat(tax_amount.toFixed(2));
    ...
    try {
      ...
      for (const item of items) {
        const lineTotal = parseFloat((parseFloat(item.unit_price) * item.quantity).toFixed(2));
        const lineTax = parseFloat((lineTotal * (parseFloat(item.tax_rate) || 0) / 100).toFixed(2));
        await txn.execute({sql:`INSERT INTO transaction_items (transaction_id,product_id,product_name,sku,quantity,unit_price,discount_amount,tax_amount,total,variation_id,variation_name) VALUES (?,?,?,?,?,?,0,?,?,?,?)`,args:[txId, item.product_id, item.product_name, item.sku || '', item.quantity, parseFloat(item.unit_price), lineTax, lineTotal, item.variation_id || null, item.variation_name || null]});
      }
      ...
      const { rows: savedItems } = await db.execute({ sql: 'SELECT * FROM transaction_items WHERE transaction_id = ?', args: [txId] }); savedTx.items = savedItems; res.status(201).json(savedTx);
    } catch(e) { if (!committed) await txn.rollback(); res.status(committed ? 500 : 400).json({ error: e.message }); }
...
router.delete('/:id/hold', requirePermission('pos_hold'), async (req, res) => {
  try {
    ...
    await db.execute({ sql: 'DELETE FROM transactions WHERE id = ?', args: [req.params.id] }); res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/', requirePermission('transactions'), async (req, res) => {
  try {
    ...
    if (source === 'online') sql += " AND t.source IN ('online','woocommerce')"; else if (source) { sql += ' AND t.source = ?'; params.push(source); }
    sql += ' ORDER BY t.created_at DESC LIMIT ?'; params.push(parseInt(limit));
    const { rows } = await db.execute({ sql, args: params }); res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const { rows: payments } = await db.execute({ sql: 'SELECT * FROM transaction_payments WHERE transaction_id = ? ORDER BY id', args: [req.params.id] }); tx.payments = payments;
    if (tx.rental_agreement_id) { const { rows: pauses } = await db.execute({ sql: 'SELECT reason, notes, started_at, ended_at FROM rental_agreement_pauses WHERE agreement_id = ? ORDER BY id', args: [tx.rental_agreement_id] }); tx.rental_pauses = pauses; }
    res.json(tx);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/', requirePermission('pos'), async (req, res) => {
  try {
    ...
    const isTaxExempt = tax_exempt ? 1 : 0; let subtotal = 0, tax_amount = 0; const processedItems = [];
    for (const item of items) {
      const { rows: [product] } = await db.execute({ sql: 'SELECT * FROM products WHERE id = ?', args: [item.product_id] });
      if (!product) throw new Error(`Product ${item.product_id} not found`);
      let variation = null, unit_price = item.uom_base_unit_price != null ? Number(item.uom_base_unit_price) : Number(product.price || 0);
      if (!Number.isFinite(unit_price) || unit_price < 0) throw new Error(`Authoritative selling price is invalid for ${product.name}`);
      if (item.variation_id) {
        ...
        if (!v) throw new Error(`Variation ${item.variation_id} not found`); variation = v;
      ...
      const lineTax = isTaxExempt ? 0 : parseFloat((lineTotal * product.tax_rate / 100).toFixed(2)); subtotal += lineTotal; tax_amount += lineTax;
      processedItems.push({ product, variation, quantity: item.quantity, unit_price, lineTotal, lineTax, discount: item.discount || 0 });
    }
    subtotal = parseFloat(subtotal.toFixed(2)); tax_amount = parseFloat(tax_amount.toFixed(2));
    const disc = parseFloat(discount_amount || 0); const storeCredit = parseFloat(store_credit_applied || 0);
    let cashBack = parseFloat(cash_back_applied || 0); let cashBackCard = null;
    if (cashBack > 0 && customer_id) {
      const { rows: [row] } = await db.execute({sql:`SELECT c.loyalty_points, c.cash_back_last_redeemed_at, cbct.points_threshold, cbct.reward_amount, cbct.min_redeem_amount, cbct.min_redeem_days, cbct.active FROM customers c LEFT JOIN cash_back_card_types cbct ON c.cash_back_card_type_id = cbct.id WHERE c.id = ?`,args:[customer_id]});
      cashBackCard = row; cashBack = Math.min(cashBack, computeAvailableCashBack(row));
    } else cashBack = 0;
    ...
    const method = tenderLegs ? (tenderLegs.length > 1 ? 'split' : tenderLegs[0].payment_method) : (payment_method || 'cash'); const isCredit = method === 'credit';
    ...
    try {
      ...
      for (const { product, variation, quantity, unit_price, lineTotal, lineTax, discount } of processedItems) {
        const itemName = variation ? `${product.name} — ${variation.name}` : product.name; const itemSku = variation ? variation.sku : product.sku;
        ...
        if (variation) await tx.execute({ sql: 'UPDATE product_variations SET stock_qty = stock_qty - ? WHERE id = ?', args: [quantity, variation.id] }); else await tx.execute({ sql: 'UPDATE products SET stock_qty = stock_qty - ? WHERE id = ?', args: [quantity, product.id] });
        if (branch_id) {
          if (req.apiKey) await tx.execute({ sql: 'UPDATE branch_inventory SET stock_qty = MAX(0, stock_qty - ?), updated_at = CURRENT_TIMESTAMP WHERE product_id = ? AND branch_id = ?', args: [quantity, product.id, branch_id] });
          else await tx.execute({ sql: `INSERT INTO branch_inventory (product_id, branch_id, stock_qty, min_stock, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(product_id, branch_id) DO UPDATE SET stock_qty = MAX(0, stock_qty - ?), updated_at = CURRENT_TIMESTAMP`, args: [product.id, branch_id, Math.max(0, product.stock_qty - quantity), product.min_stock, quantity] });
          await syncBinQty(tx, product.id, branch_id, -quantity);
        }
      }
      if (customer_id) {
        const loyaltyPts = Math.floor(total * 0.5); await tx.execute({ sql: 'UPDATE customers SET loyalty_points = loyalty_points + ?, total_spent = total_spent + ? WHERE id = ?', args: [loyaltyPts, total, customer_id] });
        if (isCredit) await tx.execute({ sql: 'UPDATE customers SET account_balance = account_balance + ? WHERE id = ?', args: [total, customer_id] });
        if (storeCredit > 0) await tx.execute({ sql: 'UPDATE customers SET account_balance = MIN(0, account_balance + ?) WHERE id = ?', args: [storeCredit, customer_id] });
        if (cashBack > 0 && cashBackCard) { const pointsToDeduct = Math.round((cashBack / cashBackCard.reward_amount) * cashBackCard.points_threshold); await tx.execute({ sql: 'UPDATE customers SET loyalty_points = MAX(0, loyalty_points - ?), cash_back_last_redeemed_at = CURRENT_TIMESTAMP WHERE id = ?', args: [pointsToDeduct, customer_id] }); }
      }
      if (quote_id) await tx.execute({ sql: `UPDATE quotations SET status = 'converted', converted_to_tx = ? WHERE id = ? AND status NOT IN ('converted','declined','cancelled')`, args: [txId, quote_id] });
...
router.get('/:id/returns', requireAuth, async (req, res) => {
  try { const { rows: returns } = await db.execute({ sql: 'SELECT * FROM returns WHERE original_transaction_id = ?', args: [req.params.id] }); for (const r of returns) { const { rows: items } = await db.execute({ sql: 'SELECT * FROM return_items WHERE return_id = ?', args: [r.id] }); r.items = items; } res.json(returns); } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/returns/:id', requireAuth, async (req, res) => {
  try {
    const { rows: [ret] } = await db.execute({ sql: `SELECT r.*, t.transaction_number as original_transaction_number,
      ...
      FROM returns r LEFT JOIN transactions t ON r.original_transaction_id = t.id LEFT JOIN customers c ON r.customer_id = c.id LEFT JOIN employees e ON r.employee_id = e.id LEFT JOIN branches b ON r.branch_id = b.id WHERE r.id = ?`, args: [req.params.id] });
    if (!ret) return res.status(404).json({ error: 'Return not found' }); const { rows: items } = await db.execute({ sql: 'SELECT * FROM return_items WHERE return_id = ?', args: [req.params.id] }); ret.items = items; res.json(ret);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

async function updateFulfillmentStatus(txId, fulfillment_status) {
  const valid = ['pending', 'processing', 'shipped', 'delivered', 'cancelled'];
  if (!valid.includes(fulfillment_status)) { const e = new Error(`Invalid status. Must be one of: ${valid.join(', ')}`); e.status = 400; throw e; }
  await db.execute({ sql: 'UPDATE transactions SET fulfillment_status = ? WHERE id = ?', args: [fulfillment_status, txId] });
  const { rows: [tx] } = await db.execute({ sql: 'SELECT * FROM transactions WHERE id = ?', args: [txId] }); if (!tx) { const e = new Error('Transaction not found'); e.status = 404; throw e; }
  if (tx.source === 'woocommerce') { const wcStatusMap = { pending: 'pending', processing: 'processing', shipped: 'on-hold', delivered: 'completed', cancelled: 'cancelled' }; try { const { rows: [map] } = await db.execute({ sql: "SELECT woo_id FROM woo_sync_map WHERE entity_type='order' AND local_id=?", args: [tx.id] }); if (map?.woo_id) { const s = await getWcSettings(); await wcRequest(s, 'PUT', `/orders/${map.woo_id}`, { status: wcStatusMap[fulfillment_status] }); } } catch(e) {} }
  return tx;
}

router.patch('/:id/fulfillment', requirePermission('transactions'), async (req, res) => { try { const tx = await updateFulfillmentStatus(req.params.id, req.body.fulfillment_status); res.json(tx); } catch(e) { res.status(e.status || 500).json({ error: e.message }); } });
...
router.post('/:id/return', requirePermission('transactions_returns'), async (req, res) => {
  try {
    ...
    if (!tx) return res.status(404).json({ error: 'Transaction not found' }); if (tx.status !== 'completed') return res.status(400).json({ error: 'Only completed transactions can be returned' });
    ...
    if (linkedAgreement) return res.status(400).json({ error: `This transaction belongs to rental agreement ${linkedAgreement.agreement_number} — process the return from the Rentals screen instead.` });
    ...
    const returnedMap = {}; alreadyReturned.forEach(r => { returnedMap[r.transaction_item_id] = r.returned_qty; });
    let returnSubtotal = 0, returnTax = 0; const processedItems = [];
    for (const item of items) {
      const txItem = txItems.find(i => i.id === item.transaction_item_id); if (!txItem) return res.status(400).json({ error: `Item ${item.transaction_item_id} not found in transaction` });
      const alreadyRet = returnedMap[item.transaction_item_id] || 0; const maxQty = txItem.quantity - alreadyRet;
      ...
      const proportion = item.quantity / txItem.quantity; const lineTotal = parseFloat((txItem.total * proportion).toFixed(2)); const lineTax = parseFloat((txItem.tax_amount * proportion).toFixed(2));
      returnSubtotal += lineTotal; returnTax += lineTax; processedItems.push({ txItem, quantity: item.quantity, lineTotal, lineTax });
    }
    returnSubtotal = parseFloat(returnSubtotal.toFixed(2)); returnTax = parseFloat(returnTax.toFixed(2)); const returnTotal = parseFloat((returnSubtotal + returnTax).toFixed(2));
    const return_number = await nextNumber(db, 'returns', 'return_number', 'RET-', 6); const retTx = await db.transaction('write'); let committed = false;
    try {
      const retResult = await retTx.execute({ sql: `INSERT INTO returns (return_number,original_transaction_id,customer_id,employee_id,branch_id,resolution,subtotal,tax_amount,total,notes) VALUES (?,?,?,?,?,?,?,?,?,?)`, args: [return_number, tx.id, tx.customer_id, employee_id || tx.employee_id, tx.branch_id, resolution, returnSubtotal, returnTax, returnTotal, notes || null] }); const retId = Number(retResult.lastInsertRowid);
      for (const { txItem, quantity, lineTotal, lineTax } of processedItems) {
        ...
        if (tx.branch_id) { await retTx.execute({ sql: `INSERT INTO branch_inventory (product_id, branch_id, stock_qty, min_stock, updated_at) VALUES (?, ?, ?, (SELECT min_stock FROM products WHERE id = ?), CURRENT_TIMESTAMP) ON CONFLICT(product_id, branch_id) DO UPDATE SET stock_qty = stock_qty + ?, updated_at = CURRENT_TIMESTAMP`, args: [txItem.product_id, tx.branch_id, quantity, txItem.product_id, quantity] }); await syncBinQty(retTx, txItem.product_id, tx.branch_id, quantity); }
      }
      if (tx.customer_id) { const loyaltyPts = Math.floor(returnTotal * 0.5); await retTx.execute({ sql: 'UPDATE customers SET loyalty_points = MAX(0, loyalty_points - ?), total_spent = MAX(0, total_spent - ?) WHERE id = ?', args: [loyaltyPts, returnTotal, tx.customer_id] }); if (resolution === 'credit_note') await retTx.execute({ sql: 'UPDATE customers SET account_balance = account_balance - ? WHERE id = ?', args: [returnTotal, tx.customer_id] }); }
      await retTx.commit(); committed = true; const { rows: [ret] } = await db.execute({ sql: 'SELECT * FROM returns WHERE id = ?', args: [retId] }); const { rows: retItems } = await db.execute({ sql: 'SELECT * FROM return_items WHERE return_id = ?', args: [retId] }); ret.items = retItems; res.status(201).json(ret);
    } catch(e) { if (!committed) await retTx.rollback(); res.status(committed ? 500 : 400).json({ error: e.message }); }
  } catch(e) { res.status(500).json({ error: e.message }); }
});

router.patch('/:id/void', requireAuth, async (req, res) => {
  try {
    const { pin, reason } = req.body; if (!pin) return res.status(400).json({ error: 'Override PIN required' });
    ...
    const authorizer = employees.find(e => { if (e.pin !== String(pin)) return false; try { const p = JSON.parse(e.permissions || '{}'); return p.void_transactions === true; } catch { return false; } });
    if (!authorizer) return res.status(403).json({ error: 'Invalid PIN or insufficient privilege' });
    const { rows: [tx] } = await db.execute({ sql: 'SELECT * FROM transactions WHERE id = ?', args: [req.params.id] }); if (!tx) return res.status(404).json({ error: 'Not found' }); if (tx.status === 'voided') return res.status(400).json({ error: 'Already voided' });
    const { rows: [linkedAgreement] } = await db.execute({ sql: 'SELECT agreement_number FROM rental_agreements WHERE checkout_transaction_id = ? OR settlement_transaction_id = ?', args: [req.params.id, req.params.id] });
    if (linkedAgreement) return res.status(400).json({ error: `This transaction belongs to rental agreement ${linkedAgreement.agreement_number} — cancel it from the Rentals screen instead (View agreement → Cancel Agreement), which correctly reverses the rental's own records.` });
    ...
    try {
      ...
      for (const item of items) { await voidTxn.execute({ sql: 'UPDATE products SET stock_qty = stock_qty + ? WHERE id = ?', args: [item.quantity, item.product_id] }); if (tx.branch_id) { await voidTxn.execute({ sql: `INSERT INTO branch_inventory (product_id, branch_id, stock_qty, min_stock, updated_at) VALUES (?, ?, ?, (SELECT min_stock FROM products WHERE id = ?), CURRENT_TIMESTAMP) ON CONFLICT(product_id, branch_id) DO UPDATE SET stock_qty = stock_qty + ?, updated_at = CURRENT_TIMESTAMP`, args: [item.product_id, tx.branch_id, item.quantity, item.product_id, item.quantity] }); await syncBinQty(voidTxn, item.product_id, tx.branch_id, item.quantity); } }
      if (tx.customer_id) { const loyaltyPts = Math.floor(tx.total * 0.5); await voidTxn.execute({ sql: 'UPDATE customers SET loyalty_points = loyalty_points - ?, total_spent = total_spent - ? WHERE id = ?', args: [loyaltyPts, tx.total, tx.customer_id] }); if (tx.is_credit) await voidTxn.execute({ sql: 'UPDATE customers SET account_balance = MAX(0, account_balance - ?) WHERE id = ?', args: [tx.total, tx.customer_id] }); }
      await voidTxn.commit();
    } catch(e) { await voidTxn.rollback(); throw e; }
