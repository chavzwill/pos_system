...
async function apiKeyAuth(req, res, next) {
  ...
  // Unknown, malformed and revoked credentials deliberately do not create one
  // persistent audit row per probe. They are rate-limited by client address to
  // prevent both credential spraying and audit-table amplification.
  if (!keyRow) {
    const verdict = consume(`invalid:${clientIp(req)}`, INVALID_MAX_PER_WINDOW, INVALID_WINDOW_MS);
    if (!verdict.allowed) return rateLimitResponse(res, verdict, INVALID_MAX_PER_WINDOW);
    return res.status(401).json({ error: 'Invalid or revoked API key' });
  ...
  if (!needed) {
    await auditApiKeyDenied(req, {
      apiKeyId: keyRow.id,
      apiKeyName: keyRow.name,
      requiredScope: null,
      control: 'api_key_endpoint_policy',
    });
    return res.status(403).json({ error: 'API keys are not permitted on this employee endpoint' });
  }
