...
async function leakageOverlay(days){
  ...
  if(await table('service_concessions')){const {rows}=await db.execute({sql:`SELECT wo.branch_id,'service_concession' category,COUNT(*) events,ROUND(SUM(CASE WHEN sc.status='approved' THEN sc.approved_amount ELSE 0 END),2) realized_loss,ROUND(SUM(CASE WHEN sc.status='pending_approval' THEN sc.proposed_amount ELSE 0 END),2) exposure FROM service_concessions sc JOIN work_orders wo ON wo.id=sc.work_order_id WHERE sc.created_at>=datetime('now',?) GROUP BY wo.branch_id`,args:[`-${days} days`]});out.push(...rows);}
