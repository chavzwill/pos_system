...
async function submitCustomerCreditChangeOnce({customerId,employeeId,branchId,departmentId,normalized}){
...
async function submitCustomerCreditChange(args){
 const normalized=normalizeProposal(args.proposal);
 const key=sha(JSON.stringify({customer_id:Number(args.customerId),employee_id:Number(args.employeeId),branch_id:args.branchId||null,department_id:Number(args.departmentId),proposal:normalized}));
 const running=submitInflight.get(key);if(running)return running;
 const promise=submitCustomerCreditChangeOnce({...args,normalized});
 submitInflight.set(key,promise);
 try{return await promise;}finally{if(submitInflight.get(key)===promise)submitInflight.delete(key);}
}
