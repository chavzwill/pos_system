...
function compile(source, filename) {
  try {
    ...
    console.error('Syntax FAILED:', filename);
...
for (const filename of browserScripts) {
  compile(fs.readFileSync(path.join(publicDir, filename), 'utf8'), filename);
}
...
if (markerIndex < 0 || scriptStart < 0 || scriptOpenEnd < 0 || scriptEnd < 0) {
  console.error('Syntax FAILED: unable to locate legacy POS application script in public/index.html');
  process.exitCode = 1;
} else {
  compile(html.slice(scriptOpenEnd + 1, scriptEnd), 'legacy-pos-app.js');
}
