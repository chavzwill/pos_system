...
async function syncLandedCosts({actorId,stats}){
  ...
  stats.landed_cost_revaluations ||= {processed:0};
  ...
  for(const allocation of allocations){
    try{
      ...
      let revaluations;
      try{
        revaluations=await finalizeLandedCostAllocation(db,allocation.id);
      }catch(e){
        stats.evidence_gaps.push({allocation_id:allocation.id,allocation_number:allocation.allocation_number,type:'landed_cost_revaluation_unresolved',automatic_posting:false,reason:e.message});
        continue;
      }
      const inventoryAdjustment=money(revaluations.reduce((s,x)=>s+Number(x.inventory_adjustment||0),0));
      const cogsAdjustment=money(revaluations.reduce((s,x)=>s+Number(x.cogs_adjustment||0),0));
      const splitTotal=money(inventoryAdjustment+cogsAdjustment);
      if(Math.abs(splitTotal-recorded)>0.01){
        stats.reconciliation_issues.push({allocation_id:allocation.id,allocation_number:allocation.allocation_number,type:'landed_cost_revaluation_split_mismatch',capitalizable_amount:recorded,inventory_adjustment:inventoryAdjustment,cogs_adjustment:cogsAdjustment});
        continue;
      }
      stats.landed_cost_revaluations.processed+=revaluations.length;
      const lines=[];
      if(inventoryAdjustment>0)lines.push({code:'1200',debit:inventoryAdjustment,credit:0,description:'Late landed cost assigned to inventory still on hand'});
      if(cogsAdjustment>0)lines.push({code:'5000',debit:cogsAdjustment,credit:0,description:'Late landed cost assigned to inventory already consumed'});
      lines.push({code:'1260',debit:0,credit:recorded,description:'Clear landed cost allocation clearing'});
      const j=await postSourceJournal({sourceType:'landed_cost_allocation',sourceId:allocation.id,sourceReference:allocation.allocation_number,entryDate:String(allocation.allocated_at||new Date().toISOString()).slice(0,10),description:`Allocate landed costs ${allocation.allocation_number} across inventory and consumed cost`,branchId:allocation.branch_id,actorId,lines});
