...
const checks=[
  ...
  ['bin-controlled stock requires exact bin evidence',route.includes('select the exact bin being written off')&&route.includes('UPDATE product_bin_assignments SET quantity=quantity-?')&&route.includes("writeoffError('WRITEOFF_STOCK_CHANGED')")],
  ['approval is independent by default',route.includes('inventory_writeoff_allow_self_approval')&&route.includes("writeoffError('WRITEOFF_SELF_APPROVAL_FORBIDDEN')")],
  ['approval rechecks live physical stock',route.includes('physicalBefore<Number(w.quantity)')&&route.includes("writeoffError('WRITEOFF_STOCK_CHANGED')")],
