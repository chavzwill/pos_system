...
const loss=read('routes/loss-control-intelligence.js'),trace=read('routes/inventory-traceability.js'),cycle=read('routes/cycle-count-hardening.js'),writeoffs=read('routes/inventory-writeoffs.js'),returns=read('routes/retail-return-hardening.js'),drawers=read('routes/drawers.js'),procurement=read('routes/procurement-outcome-intelligence.js');
new vm.Script(loss,{filename:'loss-control-intelligence.js'});
const checks=[
 ...
 ['loss case lifecycle has append-only events',loss.includes('loss_control_case_events')&&loss.includes("event_type,'detected'")||loss.includes("'detected',actor")],
 ...
 ['below-cost sales use inventory cost and effective revenue',loss.includes("signal_type:'below_cost_sale'")&&loss.includes('line_discount')&&loss.includes('allocated_order_discount')&&loss.includes('effective_revenue')],
