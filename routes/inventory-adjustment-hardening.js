    ...
    try{
      ...
      if(branchId){
        ...
        if(adjustment<0){
          ...
          if(state.available+1e-9<Math.abs(adjustment))throw Object.assign(new Error(`Only ${state.available} ${normalized.base_uom} of unrestricted, unreserved stock can be reduced; ${state.restricted} restricted and ${state.reserved||0} reserved ${normalized.base_uom} are protected`),{status:409});
        }
        ...
        if(bins.length){
          ...
          if(Number(selectedBin.quantity||0)+adjustment<-1e-9)throw Object.assign(new Error(`Adjustment would make the selected bin negative (${Number(selectedBin.quantity||0)} ${normalized.base_uom} in bin)`),{status:409});
        }else if(binId)throw Object.assign(new Error('Selected product is not assigned to that bin'),{status:409});
        const newQty=currentQty+adjustment;
        if(newQty<-1e-9)throw Object.assign(new Error(`Adjustment would make branch stock negative (${currentQty} ${normalized.base_uom} on hand)`),{status:409});
        ...
        if(currentQty+adjustment<-1e-9)throw Object.assign(new Error(`Adjustment would make global stock negative (${currentQty} ${normalized.base_uom} available)`),{status:409});
      }
      ...
      res.json({stock_qty:branchId?currentQty+adjustment:newGlobal,global_stock_qty:newGlobal,branch_id:branchId||null,bin_id:selectedBin?.bin_id||null,movement_id:movementId,base_uom:normalized.base_uom,precision:normalized.precision});
