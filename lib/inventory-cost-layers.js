...
async function ensureInventoryCostLayers() {
  ...
  readyPromise = (async () => {
    ...
    // Existing on-hand inventory predates this valuation model. Preserve it as
    // an explicit unknown-cost opening quantity rather than manufacturing FIFO
    // history from today's mutable product.cost field.
    await db.execute({
    ...
    // The old trigger snapshots the mutable catalog cost. Replace it with the
    // evidence-aware pool trigger below; legacy/uncovered sales still fall back
    // to product.cost, but are explicitly recorded as evidence gaps.
