...
function err(code,message,status=409,details=null){const e=new Error(message);e.code=code;e.status=status;if(details)e.details=details;return e;}
async function resolveCanonicalProduct(productId,executor=db){
  ...
  const {rows:[product]}=await executor.execute({sql:`SELECT p.id,p.sku,p.name,p.active,c.survivor_product_id,
    sp.sku survivor_sku,sp.name survivor_name,sp.active survivor_active
    FROM products p
    LEFT JOIN catalog_product_consolidations c ON c.duplicate_product_id=p.id
    LEFT JOIN products sp ON sp.id=c.survivor_product_id
    WHERE p.id=? LIMIT 1`,args:[id]});
  if(!product)throw err('CATALOG_PRODUCT_NOT_FOUND','Product not found.',404);
  if(product.survivor_product_id){
    return{requested_product_id:id,is_consolidated:true,canonical_product_id:Number(product.survivor_product_id),canonical_sku:product.survivor_sku||null,canonical_name:product.survivor_name||null,canonical_active:Boolean(product.survivor_active)};
  }
  return{requested_product_id:id,is_consolidated:false,canonical_product_id:id,canonical_sku:product.sku||null,canonical_name:product.name||null,canonical_active:Boolean(product.active)};
}
async function assertProductNotConsolidated(productId,executor=db){
  const resolved=await resolveCanonicalProduct(productId,executor);
  if(resolved.is_consolidated)throw err('CATALOG_PRODUCT_CONSOLIDATED',`This product was consolidated into ${resolved.canonical_name||resolved.canonical_sku||'another product'}. Use the surviving catalog record instead.`,409,resolved);
  return resolved;
}
...
async function setProductLifecycle(productId,{status,reason,actorEmployeeId=null,requestId=null,method=null,path=null}={}){
  ...
  await assertProductNotConsolidated(id);
...
async function consolidateDuplicateProducts({candidateKey,survivorProductId,reason,confirmation,expectedReviewVersion,actorEmployeeId,requestId=null,method=null,path=null}={}){
  ...
  for(const id of duplicates){
    ...
    const {rows:[v]}=await db.execute({sql:'SELECT COUNT(*) c FROM product_variations WHERE product_id=? AND ABS(COALESCE(stock_qty,0))>0.000000001',args:[id]});
    if(Number(v?.c||0)>0)throw err('CATALOG_CONSOLIDATION_VARIATION_STOCK_PRESENT','A retiring duplicate still has variation stock. Move, sell, return, adjust, or write off that variation stock before consolidating.',409);
  ...
  try{
    ...
    for(const id of duplicates){
      const {rows:[variationStock]}=await tx.execute({sql:'SELECT COUNT(*) c FROM product_variations WHERE product_id=? AND ABS(COALESCE(stock_qty,0))>0.000000001',args:[id]});
      if(Number(variationStock?.c||0)>0)throw err('CATALOG_CONSOLIDATION_VARIATION_STOCK_PRESENT','Variation stock changed while consolidation was being prepared. Review the duplicate and try again.',409);
      ...
      await tx.execute({sql:`INSERT OR IGNORE INTO promotion_items(promotion_id,item_type,item_id)
        SELECT promotion_id,'product',? FROM promotion_items WHERE item_type='product' AND item_id=?`,args:[survivor,id]});
      await tx.execute({sql:"DELETE FROM promotion_items WHERE item_type='product' AND item_id=?",args:[id]});
...
function categoryCandidateKey(ids=[]){return `duplicate_category_name:${[...new Set(ids.map(Number).filter(Number.isInteger))].sort((a,b)=>a-b).join(',')}`;}
async function resolveCanonicalCategory(categoryId,executor=db){
  const id=Number(categoryId);if(!Number.isInteger(id)||id<=0)throw err('CATALOG_CATEGORY_ID_INVALID','A valid category is required.',400);
  const {rows:[live]}=await executor.execute({sql:'SELECT id,name,description FROM categories WHERE id=?',args:[id]});
  if(live)return{requested_category_id:id,is_consolidated:false,canonical_category_id:id,canonical_name:live.name,canonical_description:live.description||null};
  const {rows:[mapped]}=await executor.execute({sql:`SELECT c.duplicate_category_id,c.duplicate_name,c.survivor_category_id,s.name survivor_name,s.description survivor_description
    FROM catalog_category_consolidations c JOIN categories s ON s.id=c.survivor_category_id WHERE c.duplicate_category_id=? LIMIT 1`,args:[id]});
  if(!mapped)throw err('CATALOG_CATEGORY_NOT_FOUND','Category not found.',404);
  return{requested_category_id:id,is_consolidated:true,canonical_category_id:Number(mapped.survivor_category_id),canonical_name:mapped.survivor_name,canonical_description:mapped.survivor_description||null,duplicate_name:mapped.duplicate_name};
}
async function inspectCategoryConsolidation(candidateKey){
  const key=String(candidateKey||'').trim();
  if(!/^duplicate_category_name:\d+(,\d+)+$/.test(key))throw err('CATALOG_CATEGORY_CONSOLIDATION_KEY_INVALID','Choose a duplicate category group.',400);
  const ids=key.split(':')[1].split(',').map(Number);
  const marks=ids.map(()=>'?').join(',');
  const {rows:cats}=await db.execute({sql:`SELECT c.id,c.name,c.description,COUNT(p.id) product_count FROM categories c LEFT JOIN products p ON p.category_id=c.id WHERE c.id IN (${marks}) GROUP BY c.id ORDER BY c.id`,args:ids});
  if(cats.length!==ids.length)throw err('CATALOG_CATEGORY_CONSOLIDATION_STALE','This duplicate category group changed. Refresh Catalog Health.',409);
  const normalized=[...new Set(cats.map(c=>normalizeProductName(c.name)))];
  if(normalized.length!==1)throw err('CATALOG_CATEGORY_CONSOLIDATION_STALE','These categories no longer have the same normalized name.',409);
  const {rows:scopes}=await db.execute({sql:`SELECT item_id,COUNT(*) promotion_count FROM promotion_items WHERE item_type='category' AND item_id IN (${marks}) GROUP BY item_id`,args:ids});
  const promo=new Map(scopes.map(x=>[Number(x.item_id),Number(x.promotion_count||0)]));
  return{candidate_key:key,categories:cats.map(c=>({id:Number(c.id),name:c.name,description:c.description||null,product_count:Number(c.product_count||0),promotion_count:promo.get(Number(c.id))||0})),read_only:true};
}
async function consolidateDuplicateCategories({candidateKey,survivorCategoryId,reason,confirmation,actorEmployeeId,requestId=null,method=null,path=null}={}){
  const key=String(candidateKey||'').trim(),survivor=Number(survivorCategoryId),why=String(reason||'').trim(),actor=Number(actorEmployeeId);
  if(String(confirmation||'').trim()!=='CONSOLIDATE')throw err('CATALOG_CATEGORY_CONSOLIDATION_CONFIRMATION_REQUIRED','Type CONSOLIDATE to confirm this category consolidation.',400);
  if(why.length<5)throw err('CATALOG_CATEGORY_CONSOLIDATION_REASON_REQUIRED','Enter a meaningful reason for this category consolidation.',400);
  if(!Number.isInteger(actor)||actor<=0)throw err('CATALOG_CATEGORY_CONSOLIDATION_ACTOR_REQUIRED','An authenticated employee is required.',401);
  if(!/^duplicate_category_name:\d+(,\d+)+$/.test(key))throw err('CATALOG_CATEGORY_CONSOLIDATION_KEY_INVALID','Choose a duplicate category group.',400);
  const {rows:existing}=await db.execute({sql:'SELECT * FROM catalog_category_consolidations WHERE candidate_key=? ORDER BY duplicate_category_id',args:[key]});
  if(existing.length){if(existing.every(x=>Number(x.survivor_category_id)===survivor))return{changed:false,survivor_category_id:survivor,removed_category_ids:existing.map(x=>Number(x.duplicate_category_id)),mappings:existing};throw err('CATALOG_CATEGORY_CONSOLIDATION_ALREADY_COMPLETED','This category group was already consolidated to a different survivor.',409);}
  const preview=await inspectCategoryConsolidation(key),ids=preview.categories.map(x=>Number(x.id));
  if(!ids.includes(survivor))throw err('CATALOG_CATEGORY_CONSOLIDATION_SURVIVOR_INVALID','Choose the surviving category from this exact duplicate group.',400);
  const duplicates=ids.filter(id=>id!==survivor);
  await ensureSecurityAuditTable();
  const tx=await db.transaction('write');let committed=false;
  try{
    const marks=ids.map(()=>'?').join(',');
    const {rows:fresh}=await tx.execute({sql:`SELECT id,name,description FROM categories WHERE id IN (${marks}) ORDER BY id`,args:ids});
    if(fresh.length!==ids.length||new Set(fresh.map(c=>normalizeProductName(c.name))).size!==1)throw err('CATALOG_CATEGORY_CONSOLIDATION_STALE','The duplicate category group changed while you were reviewing it.',409);
    for(const id of duplicates){
      const source=fresh.find(x=>Number(x.id)===id);
      await tx.execute({sql:'UPDATE products SET category_id=? WHERE category_id=?',args:[survivor,id]});
      await tx.execute({sql:`INSERT OR IGNORE INTO promotion_items(promotion_id,item_type,item_id) SELECT promotion_id,'category',? FROM promotion_items WHERE item_type='category' AND item_id=?`,args:[survivor,id]});
      await tx.execute({sql:"DELETE FROM promotion_items WHERE item_type='category' AND item_id=?",args:[id]});
      await tx.execute({sql:'INSERT INTO catalog_category_consolidations(candidate_key,survivor_category_id,duplicate_category_id,duplicate_name,reason,consolidated_by) VALUES(?,?,?,?,?,?)',args:[key,survivor,id,source.name,why,actor]});
      const deleted=await tx.execute({sql:'DELETE FROM categories WHERE id=?',args:[id]});
      if(Number(deleted.rowsAffected||0)!==1)throw err('CATALOG_CATEGORY_CONSOLIDATION_STALE','A duplicate category changed during consolidation.',409);
    }
    await recordSecurityAudit({actorEmployeeId:actor,action:'catalog_category_consolidated',targetType:'catalog_category_candidate',targetId:key,oldValue:{category_ids:ids},newValue:{survivor_category_id:survivor,removed_category_ids:duplicates},reason:why,requestId,method,path,control:'catalog_category_consolidation',executor:tx});
    const {rows:mappings}=await tx.execute({sql:'SELECT * FROM catalog_category_consolidations WHERE candidate_key=? ORDER BY duplicate_category_id',args:[key]});
    await tx.commit();committed=true;return{changed:true,survivor_category_id:survivor,removed_category_ids:duplicates,mappings};
  }catch(error){if(!committed)try{await tx.rollback();}catch(_){}throw error;}
}
async function catalogHealth({limit=100}={}){
  ...
  for(const c of dupes){const ids=String(c.ids).split(',').map(Number);issues.push(issue('duplicate_category_name','medium','Duplicate category names','Duplicate categories split reporting and make product selection harder.',{type:'category',name:c.name,ids,candidate_key:categoryCandidateKey(ids)},'Review the duplicate categories, choose the correct surviving category, and use the controlled consolidation action.'));}
...
module.exports={resolveCanonicalProduct,assertProductNotConsolidated,resolveCanonicalCategory,inspectCategoryConsolidation,consolidateDuplicateCategories,categoryCandidateKey,inspectProductCleanup,archiveProduct,setProductLifecycle,reviewDuplicateCandidate,planDuplicateConsolidation,inspectDuplicateConsolidation,consolidateDuplicateProducts,catalogHealth,buildRemediationQueue,recordKey,duplicateCandidateKey,duplicateProductIssues,normalizeProductName,normalizeSku};
