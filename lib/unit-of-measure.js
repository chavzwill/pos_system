...
async function resolveProductUom(executor,productId,uomCode,mode='sell'){
  ...
  if(requested===code(profile.base_uom))return {uom_code:requested,uom_name:requested,factor_to_base:1,profile};
  const permissionColumn=mode==='purchase'?'purchase_allowed':'sell_allowed';
  const {rows:[row]}=await executor.execute({sql:`SELECT * FROM product_uom_conversions WHERE product_id=? AND uom_code=? AND active=1 AND ${permissionColumn}=1`,args:[productId,requested]});
...
async function snapshot(executor,{sourceType,sourceId,sourceLineId,productId,enteredQuantity,resolved,baseQuantity}){
  await executor.execute({sql:`INSERT INTO uom_usage_snapshots(source_type,source_id,source_line_id,product_id,entered_quantity,entered_uom,factor_to_base,base_quantity,base_uom) VALUES(?,?,?,?,?,?,?,?,?)`,args:[sourceType,String(sourceId),sourceLineId==null?null:String(sourceLineId),productId,enteredQuantity,resolved.uom_code,resolved.factor_to_base,baseQuantity,resolved.profile.base_uom]});
}
module.exports={STANDARD_UNITS,code,standardDimension,convertStandard,ensureUomSchema,getProfile,resolveProductUom,toBaseQuantity,snapshot};
