...
async function syncSourceEvidence(tx,job,stage){
 ...
 if(job.source_type==='sales_invoice'){
  if(stage==='completed') await tx.execute({sql:`UPDATE transactions SET fulfillment_status='delivered' WHERE id=?`,args:[job.source_id]}).catch(()=>{});
  await event(tx,job.id,'sales_logistics_sync',null,null,stage==='completed'?`${text}; sales fulfillment marked delivered`:text,null);
 }
...
router.get('/jobs/:id/execution',requireAnyPermission('transfers','rentals','purchasing','transactions'),async(req,res)=>{try{const job=await getJob(req.params.id);if(!job)return res.status(404).json({error:'Dispatch job not found'});const execution=await getExecution(req.params.id);const {rows:custody}=await db.execute({sql:'SELECT * FROM dispatch_custody_events WHERE dispatch_job_id=? ORDER BY created_at,id',args:[req.params.id]});const {rows:proofs}=await db.execute({sql:'SELECT * FROM dispatch_proofs WHERE dispatch_job_id=? ORDER BY captured_at,id',args:[req.params.id]});let journey_context=null;try{const {rows:[ctx]}=await db.execute({sql:`SELECT dr.route_number,dr.status route_status,drs.stop_sequence,drs.planned_arrival,ol.geocode_status origin_geocode_status,ol.latitude origin_latitude,ol.longitude origin_longitude,dl.geocode_status destination_geocode_status,dl.latitude destination_latitude,dl.longitude destination_longitude FROM dispatch_jobs dj LEFT JOIN dispatch_route_stops drs ON drs.dispatch_job_id=dj.id LEFT JOIN dispatch_routes dr ON dr.id=drs.route_id LEFT JOIN dispatch_job_locations djl ON djl.dispatch_job_id=dj.id LEFT JOIN dispatch_locations ol ON ol.id=djl.origin_location_id LEFT JOIN dispatch_locations dl ON dl.id=djl.destination_location_id WHERE dj.id=? LIMIT 1`,args:[req.params.id]});journey_context=ctx||null;}catch(_){}res.json({job,execution,custody,proofs,journey_context});}catch(e){res.status(500).json({error:e.message});}});
...
router.post('/jobs/:id/reschedule',requireAnyPermission('transfers'),async(req,res)=>{try{const job=await getJob(req.params.id),execution=await getExecution(req.params.id);if(!job||!execution)return res.status(404).json({error:'Dispatch execution not found'});if(execution.stage!=='failed')return res.status(409).json({error:'Only a failed attempt can be rescheduled'});if(!req.body?.scheduled_for)return res.status(400).json({error:'scheduled_for is required'});const {rows:[vehicle]}=await db.execute({sql:'SELECT id,status,active FROM dispatch_vehicles WHERE id=?',args:[execution.vehicle_id]});if(!vehicle||!vehicle.active||vehicle.status!=='available')return res.status(409).json({error:'Previously assigned vehicle is no longer available; reassign the dispatch instead'});const {rows:[conflict]}=await db.execute({sql:`SELECT dj.job_number FROM dispatch_executions de JOIN dispatch_jobs dj ON dj.id=de.dispatch_job_id WHERE (de.driver_employee_id=? OR de.vehicle_id=?) AND de.stage NOT IN ('completed','failed','cancelled') AND de.dispatch_job_id!=? LIMIT 1`,args:[execution.driver_employee_id,execution.vehicle_id,job.id]});if(conflict)return res.status(409).json({error:`Driver or vehicle is now committed to ${conflict.job_number}; reassign before rescheduling`});const tx=await db.transaction('write');try{await tx.execute({sql:`UPDATE dispatch_executions SET stage='rescheduled',rescheduled_for=?,failure_reason=NULL,updated_by_employee_id=?,updated_at=CURRENT_TIMESTAMP WHERE dispatch_job_id=?`,args:[req.body.scheduled_for,actor(req),job.id]});await tx.execute({sql:`UPDATE dispatch_jobs SET status='scheduled',scheduled_for=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,args:[req.body.scheduled_for,job.id]});await tx.execute({sql:`UPDATE dispatch_vehicles SET status='assigned',updated_at=CURRENT_TIMESTAMP WHERE id=?`,args:[execution.vehicle_id]});await event(tx,job.id,'field_rescheduled',job.status,'scheduled',`Rescheduled for ${req.body.scheduled_for}`,actor(req));await tx.commit();}catch(e){await tx.rollback();throw e;}res.json(await getExecution(job.id));}catch(e){res.status(400).json({error:e.message});}});
