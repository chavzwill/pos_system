...
function networkRule(url,method,status){if(status<200||status>=300)return'';const t=task(),s=step();if(!t||!s)return'';
 ...
 if(t==='Create or approve a purchase request'&&s===3&&/\/api\/purchase-requests\//.test(url)&&['POST','PATCH','PUT'].includes(method))return'Purchase request decision saved.';
 if(t==='Create or approve a purchase request'&&s===4&&/\/api\/purchase-(?:requests|orders)\//.test(url)&&['POST','PATCH','PUT'].includes(method))return'Purchase order conversion completed.';
 if(t==='Create, edit, copy, cancel or receive a PO'&&s===2&&method==='POST'&&/\/api\/purchase-orders(?:\?|$)/.test(url))return'Purchase order created.';
 if(t==='Create, edit, copy, cancel or receive a PO'&&s===3&&/\/api\/purchase-orders\//.test(url)&&['POST','PATCH','PUT','DELETE'].includes(method))return'Purchase order change saved.';
 if(t==='Create, edit, copy, cancel or receive a PO'&&s===4&&/\/api\/purchase-orders\//.test(url)&&/receive/i.test(url)&&['POST','PATCH','PUT'].includes(method))return'Receiving was recorded.';
 if(t==='Create, dispatch or receive a branch transfer'&&s===2&&method==='POST'&&/\/api\/.*transfers/.test(url))return'Transfer created.';
 if(t==='Create, dispatch or receive a branch transfer'&&s===3&&/\/api\/.*transfers\//.test(url)&&/dispatch/i.test(url)&&['POST','PATCH','PUT'].includes(method))return'Transfer dispatched.';
 if(t==='Create, dispatch or receive a branch transfer'&&s===4&&/\/api\/.*transfers\//.test(url)&&/receive/i.test(url)&&['POST','PATCH','PUT'].includes(method))return'Transfer received.';
 if(t==='Adjust inventory'&&s>=2&&/\/api\/.*(?:inventory|stock|products)/.test(url)&&['POST','PATCH','PUT'].includes(method))return'Inventory change saved.';
 if(t==='Create or manage a quotation'&&s===2&&method==='POST'&&/\/api\/.*quot/.test(url))return'Quotation created.';
 if(t==='Create or manage a rental'&&s>=2&&/\/api\/.*rental/.test(url)&&['POST','PATCH','PUT'].includes(method))return'Rental workflow updated.';
 if(t==='Create or work a repair'&&s>=2&&/\/api\/.*(?:repair|work-orders)/.test(url)&&['POST','PATCH','PUT'].includes(method))return'Repair workflow updated.';
 if(t==='Dispatch, route or complete a delivery'&&s>=3&&/\/api\/.*(?:dispatch|logistics)/.test(url)&&['POST','PATCH','PUT'].includes(method))return'Dispatch workflow updated.';
 return'';}
...
function domCompletion(){if(advancing||!root())return;const t=task(),s=step();if(t==='Complete a sale'&&s===2&&document.querySelector('#tt-sales-workspace .tt-sales__line'))advance('Product added to the sale.');
...
let raf=0;const observer=new MutationObserver(()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;domCompletion();});});observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','hidden','aria-selected']});
