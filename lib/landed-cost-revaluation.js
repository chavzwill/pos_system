...
async function finalizeLandedCostAllocationItem(executor, allocationItemId){
  ...
  const {rows:[item]}=await executor.execute({sql:`SELECT lcai.*,lca.id allocation_id,lca.allocated_at,lca.allocation_number,
      l.product_id layer_product_id,l.branch_key,l.quantity_original,l.received_at
  ...
  if(!productId||!Number.isFinite(amount)||amount<0||!Number.isFinite(receiptQty)||receiptQty<=0) throw new Error('Invalid landed-cost revaluation evidence');
  ...
  for(const ev of events){
    if(ev.kind==='receipt'){
      ...
      if(ev.id===Number(item.id ? 0 : 0)){}
      if(ev.id===Number((layers.find(l=>Number(l.id)===Number(item.id))||{}).id)){}
      // The target layer is identified by its purchase-receipt line below, not by allocation-item id.
      const targetLayer=layers.find(l=>String(l.received_at)===String(item.received_at) && Number(l.quantity_original||0)===Number(item.quantity_original||0));
      if(targetLayer && ev.id===Number(targetLayer.id) && !active){ active=true; deltaRemaining=amount; }
      continue;
    }
  ...
  // If replay could not activate the exact receipt layer, fail closed instead of guessing.
  ...
  if(inventoryAdjustment>0){
    await executor.execute({sql:`UPDATE inventory_cost_pools
