...
router.get('/:id/po-template',async(req,res)=>{try{const doc=await hydrate(Number(req.params.id));if(!doc)return res.status(404).json({error:'Quote import not found'});if(doc.converted_po_id)return res.status(409).json({error:`This supplier quote is already linked to purchase order ${doc.converted_po_number||doc.converted_po_id}`});const unresolved=doc.lines.filter(x=>!x.product_id||x.match_status!=='matched');if(unresolved.length)return res.status(409).json({error:`${unresolved.length} quote line(s) still need a confirmed inventory match or inventory item creation`});res.json({source_quote_import_id:doc.id,supplier_id:doc.supplier_id,items:doc.lines.map(x=>({product_id:x.product_id,product_name:x.product_name,product_sku:x.product_sku,quantity_ordered:Number(x.entered_quantity),unit_cost:Number(x.entered_unit_price),uom_code:x.entered_uom||undefined,supplier_part_number:x.supplier_part_number||undefined,source_description:x.description}))})}catch(e){res.status(500).json({error:e.message})}});
