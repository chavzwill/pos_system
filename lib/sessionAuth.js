...
async function sessionAuth(req, res, next) {
  ...
  try {
    ...
    // Administrative credential reset is deliberately stronger than ordinary
    // session authentication. Possession of a live privileged session alone is
    // insufficient: the acting employee must re-enter their own password for
    // this exact request. This blocks credential takeover from an unattended or
    // stolen admin session and runs before the route performs any mutation.
    if (privilegedPasswordResetRequest(req)) {
      if (!can(emp.permissions, 'security_manage')) {
        return res.status(403).json({ error: 'Missing permission: security_manage' });
      }
      if (!(await verifyElevatedPasswordReauthentication(req, emp))) {
        return res.status(403).json({
          error: 'Elevated reauthentication is required for employee password reset',
