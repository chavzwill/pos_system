...
async function sessionAuth(req, res, next) {
  ...
  try {
    const { rows: [session] } = await db.execute({
      sql: `SELECT * FROM sessions
        WHERE token_hash = ?
          ...
          AND datetime(created_at, '+12 hours') > datetime('now')`,
      args: [hashToken(raw)],
    });
    if (!session) {
    ...
    // Keep all lifetime arithmetic in SQLite's UTC datetime representation.
    // Active use can slide the idle deadline, but never past created_at + 12h.
    db.execute({
      sql: `UPDATE sessions
        SET last_seen_at = datetime('now'),
            expires_at = CASE
              WHEN datetime('now', '+8 hours') < datetime(created_at, '+12 hours')
                THEN datetime('now', '+8 hours')
              ELSE datetime(created_at, '+12 hours')
            END
