(()=>{'use strict';
const VERSION='20260909-stability-2';
const assets={
...
function ensureCss(href,force=false){
  if(!force&&[...document.styleSheets].some(x=>x.href&&assetPath(x.href)===href))return Promise.resolve();
  if(!force&&cssLoading.has(href))return cssLoading.get(href);
  const p=new Promise((resolve,reject)=>{
    if(force)document.querySelectorAll('link[rel="stylesheet"]').forEach(l=>{if(assetPath(l.href)===href)l.remove();});
    const l=document.createElement('link');l.rel='stylesheet';l.href=`${href}?v=${VERSION}${force?'&retry='+Date.now():''}`;l.dataset.ttWorkspaceCss=href;
    const timer=setTimeout(()=>{l.remove();reject(new Error(`Timed out loading ${href}`));},8000);
    l.onload=()=>{clearTimeout(timer);resolve();};l.onerror=()=>{clearTimeout(timer);l.remove();reject(new Error(`Unable to load ${href}`));};document.head.appendChild(l);
  });
  cssLoading.set(href,p);
  // Do not use an ignored Promise.finally() here: the returned promise would
  // itself reject and surface as an unhandledrejection in the shell.
  p.then(()=>cssLoading.delete(href),()=>cssLoading.delete(href));
  return p;
}
...
function loadScript(src,global,force=false){
  if(window[global]?.open)return Promise.resolve(window[global]);
  const key=src+'|'+global;if(!force&&loading.has(key))return loading.get(key);
  ...
  loading.set(key,p);
  // Important: Promise.prototype.finally returns a second promise. Ignoring it
  // caused a rejected workspace load to generate an unhandled rejection even
  // when the original load was caught and recovered. That shell-wide rejection
  // could trigger the page-crash protection seen by operators.
  p.then(()=>loading.delete(key),()=>loading.delete(key));
  return p;
}
...
function recovery(title,error,retry){const old=document.getElementById('tt-workspace-load-error');old?.remove();const host=document.createElement('div');host.id='tt-workspace-load-error';host.className='tt-workspace-load-error';host.innerHTML=`<section role="alertdialog" aria-modal="true" aria-labelledby="tt-workspace-error-title"><span>Workspace recovery</span><h2 id="tt-workspace-error-title"></h2><p></p><div><button type="button" data-retry>Retry workspace</button><button type="button" data-close>Return to dashboard</button></div><small>No business action has been retried automatically.</small></section>`;host.querySelector('h2').textContent=`${title} could not open`;host.querySelector('p').textContent=error?.message||'The workspace failed to initialize.';host.querySelector('[data-close]').onclick=()=>host.remove();host.querySelector('[data-retry]').onclick=()=>{host.remove();retry();};document.body.appendChild(host);host.querySelector('[data-retry]')?.focus();}
function dispatchHealth(detail){window.dispatchEvent(new CustomEvent('tt:workspace-health',{detail}));}
async function openFeature(key,title='Workspace'){
  if(key==='legacy'){location.href='/legacy?from=shell&open='+encodeURIComponent(title);return true;}
  const a=assets[key];if(!a)throw new Error(`Unknown workspace module: ${key}`);const [css,js,global]=a;
  dispatchHealth({key,title,state:'loading',at:Date.now()});
  let api;
  try{await ensureCss(css,false);api=await loadScript(js,global,false);}catch(first){console.warn('Workspace first load failed; retrying cleanly',key,first);await ensureCss(css,true);api=await loadScript(js,global,true);}
  if(!api?.open)throw new Error(`${title} did not initialize after a clean reload.`);
  try{if(key==='customer-crm-workspace'&&/pipeline/i.test(title))await api.open({tab:'pipeline'});else await api.open();}
  catch(error){dispatchHealth({key,title,state:'failed',message:error?.message||String(error),at:Date.now()});throw error;}
  document.documentElement.dataset.ttActiveWorkspace=key;dispatchHealth({key,title,state:'open',at:Date.now()});void loadExtras(key);return true;
}
...
async function certify(){
  const unique=[...new Set(Object.values(assets).flatMap(([css,js])=>[css,js]).concat(Object.values(extras).flat()))];
  const results=[];for(const url of unique)results.push(await probe(url));
  const failed=results.filter(x=>!x.ok);const report={ok:failed.length===0,checked:results.length,failed,results,at:new Date().toISOString()};
  dispatchHealth({state:'certification',...report});return report;
}
window.TotalToolsShellOpen=openFeature;
window.TotalToolsWorkspaceLoader={open:openFeature,assets,extras,certify,version:VERSION};
document.addEventListener('click',e=>{const b=e.target.closest?.('#shell-root [data-open]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();const key=b.dataset.open,title=b.dataset.title||'Workspace';if(b.dataset.ttOpening==='1')return;b.dataset.ttOpening='1';b.disabled=true;openFeature(key,title).catch(err=>recovery(title,err,()=>openFeature(key,title).catch(error=>recovery(title,error,()=>location.reload())))).finally(()=>{delete b.dataset.ttOpening;b.disabled=false;});},true);
})();