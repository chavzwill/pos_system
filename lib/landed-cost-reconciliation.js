...
async function reconcileSupplierInvoiceLandedCosts(executor,invoiceId,{actorId=null}={}){
  ...
  const allocationInsert=await executor.execute({sql:`INSERT OR IGNORE INTO landed_cost_allocations(allocation_number,supplier_invoice_id,purchase_order_id,branch_id,capitalizable_amount,allocation_basis,allocated_by_employee_id)
    VALUES(?,?,?,?,?,?,?)`,args:[allocationNumber,invoice.id,invoice.purchase_order_id,invoice.branch_id||receiptItems[0].branch_id||null,expected,basis,actorId]});
  ...
  for(let i=0;i<receiptItems.length;i++){
    ...
    await executor.execute({sql:`INSERT OR IGNORE INTO landed_cost_allocation_items(allocation_id,purchase_receipt_item_id,receipt_id,product_id,quantity_received,original_unit_cost,allocated_amount,landed_cost_per_unit,adjusted_unit_cost)
      VALUES(?,?,?,?,?,?,?,?,?)`,args:[allocationId,r.purchase_receipt_item_id,r.receipt_id,r.product_id,qty,original,allocated,per,adjusted]});
  }
