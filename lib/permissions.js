...
const { auditPermissionDenied } = require('./securityAudit');
const PERMISSION_TREE = [
  ...
  { key: 'inventory', subs: [{ key: 'inventory_add' }, { key: 'inventory_edit' }, { key: 'inventory_delete' }, { key: 'inventory_adjust' }, { key: 'inventory_disposition' }, { key: 'inventory_writeoff_create' }, { key: 'inventory_writeoff_approve' }, { key: 'inventory_media_bulk' }] },
  ...
  { key: 'accounts', subs: [{ key: 'accounts_create' }, { key: 'accounts_payments' }, { key: 'accounts_writeoff' }, { key: 'accounts_credit_approve' }] },
...
const EXPLICIT_ONLY_PERMISSIONS = new Set(['customers_sensitive', 'settings_integrations', 'accounts_credit_approve', 'inventory_media_bulk', 'rentals_compliance_exception']);
...
function denyWithAudit(req,res,{status=403,error,requiredPermissions=[],control='rbac'}={}){
  if(status===403&&req.employee?.id)auditPermissionDenied(req,{requiredPermissions,control}).catch(()=>{});
  res.status(status).json({error,...(control?{control}: {})});
  return 'denied';
}
...
function enforceDispatch(req,res){
  ...
  if(required==='source_document'){
    ...
    return denyWithAudit(req,res,{error:'Missing permission to view this Dispatch source document',requiredPermissions:['dispatch_view','dispatch_plan','dispatch_execute','dispatch_admin','purchasing','transactions','rentals','work_orders'],control:'dispatch_rbac'});
  }
  if(required==='dispatch_unclassified'){
    return denyWithAudit(req,res,{error:'Dispatch mutation is not classified for RBAC',requiredPermissions:['classified_dispatch_permission'],control:'dispatch_rbac_unclassified'});
  }
  ...
  return denyWithAudit(req,res,{error:`Missing permission: ${required}`,requiredPermissions:allowed,control:'dispatch_rbac'});
}
...
function requirePermission(key) {
  return (req, res, next) => {
    ...
    if (req.apiKey) return next();
    if (!req.employee) return res.status(401).json({ error: 'Authentication required' });
    if (!can(req.employee.permissions, key)) { denyWithAudit(req,res,{error:`Missing permission: ${key}`,requiredPermissions:[key],control:'rbac'}); return; }
    next();
  };
}
function requireAnyPermission(...keys) {
  return (req, res, next) => {
    ...
    if (req.apiKey) return next();
    if (!req.employee) return res.status(401).json({ error: 'Authentication required' });
    if (!keys.some(key => can(req.employee.permissions, key))) { denyWithAudit(req,res,{error:`Missing permission: one of ${keys.join(', ')}`,requiredPermissions:keys,control:'rbac'}); return; }
    next();
  };
}
