  ...
  try{
    ...
    try{
      ...
      for(const x of validated){
        const {item,qty,unitCost,lineCost,identity}=x;
      ...
      await tx.execute({sql:'UPDATE purchase_orders SET status=?,received_at=? WHERE id=?',args:[newStatus,allReceived?new Date().toISOString():po.received_at,po.id]});
      if(allReceived)await tx.execute({sql:`UPDATE purchase_requests SET status='received' WHERE converted_to_po_id=? AND status!='received'`,args:[po.id]});
