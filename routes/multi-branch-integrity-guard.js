...
async function binBranch(binId){const {rows:[row]}=await db.execute({sql:`SELECT COALESCE(sb.branch_id,z.branch_id) branch_id FROM storage_bins sb LEFT JOIN warehouse_zones z ON z.id=sb.zone_id WHERE sb.id=?`,args:[binId]});return row?row.branch_id:null;}
async function assignmentRecord(id){const {rows:[row]}=await db.execute({sql:`SELECT a.*,COALESCE(a.branch_id,sb.branch_id,z.branch_id) resolved_branch_id FROM product_bin_assignments a LEFT JOIN storage_bins sb ON sb.id=a.bin_id LEFT JOIN warehouse_zones z ON z.id=sb.zone_id WHERE a.id=?`,args:[id]});return row||null;}
...
router.use(async(req,res,next)=>{
  try{
    ...
    if(p==='/warehouse/assignments'&&req.method==='POST'){
      ...
      req.body ||= {};req.body.quantity=0;req.body.branch_id=resolvedBranch;return next();
    }
    ...
    if(assignmentId&&req.method==='PUT'){
      ...
      const updated=await assignmentRecord(assignmentId);return res.json(updated);
    }
    ...
    if(p==='/transactions'&&req.method==='POST'){if(!assertBranch(req,res,req.body?.branch_id))return;req.body ||= {};req.body.employee_id=req.employee.id;return next();}
