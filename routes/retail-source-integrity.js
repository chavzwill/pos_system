...
router.post('/transactions',async(req,res,next)=>{
  try{
    ...
    if(quoteId){
      const {rows:[row]}=await db.execute({sql:'SELECT id,quote_number,status,quote_type,branch_id,customer_id,converted_to_tx,converted_to_agreement_id FROM quotations WHERE id=?',args:[quoteId]});
      ...
      if(row.status==='converted'||row.converted_to_tx){
        ...
        return res.status(409).json({error:'Quotation must be accepted before checkout',control:'retail_source_integrity'});
      }
    ...
    if(held&&quote&&quote.converted_to_tx&&String(quote.converted_to_tx)!==String(held.id)){
      return res.status(409).json({error:'Held sale does not belong to the supplied quotation',control:'retail_source_integrity'});
    }
    next();
