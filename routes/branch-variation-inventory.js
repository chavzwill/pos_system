...
router.get('/migration-status',requirePermission('inventory'),async(req,res)=>{
  try{
    ...
    const {rows:variations}=await db.execute({sql:'SELECT id,name,sku,active,stock_qty FROM product_variations WHERE product_id=? ORDER BY name,id',args:[productId]});
    if(!variations.length)return res.status(409).json({error:'Product has no catalog variations'});
    const {rows:branches}=await db.execute({sql:`SELECT b.id,b.name,COALESCE(bi.stock_qty,0) parent_branch_qty,m.status,m.baseline_total,m.counted_at,m.finalized_at,m.reason
      FROM branches b LEFT JOIN branch_inventory bi ON bi.branch_id=b.id AND bi.product_id=?
      LEFT JOIN branch_variation_migrations m ON m.product_id=? AND m.branch_id=b.id
      WHERE b.active=1 OR COALESCE(bi.stock_qty,0)<>0 ORDER BY b.name,b.id`,args:[productId,productId]});
    const {rows:ledger}=await db.execute({sql:`SELECT bvi.branch_id,bvi.variation_id,bvi.stock_qty FROM branch_variation_inventory bvi WHERE bvi.product_id=?`,args:[productId]});
    ...
    const counted=branchRows.filter(b=>b.status).length;
    const required=branchRows.filter(b=>Math.abs(Number(b.parent_branch_qty||0))>1e-9||b.status).length;
...
router.post('/opening-balance',requirePermission('inventory_edit'),async(req,res)=>{
  try{
    ...
    if(!productId||!branchId)return res.status(400).json({error:'product_id and branch_id are required'});
    if(reason.length<8)return res.status(400).json({error:'A specific physical-count reason of at least 8 characters is required'});
    ...
    const {rows:[parent]}=await db.execute({sql:'SELECT stock_qty FROM branch_inventory WHERE product_id=? AND branch_id=?',args:[productId,branchId]});const parentQty=Number(parent?.stock_qty||0);
    const total=Number([...provided.values()].reduce((s,n)=>s+n,0).toFixed(6));
    if(Math.abs(total-parentQty)>1e-6)return res.status(409).json({error:`Physical variation count (${total}) must equal the authoritative branch product quantity (${parentQty}) before migration. Resolve the parent stock discrepancy first.`,control:'branch_variation_parent_mismatch',parent_branch_qty:parentQty,variation_total:total});
    const tx=await db.transaction('write');let committed=false;
    try{
      const {rows:[again]}=await tx.execute({sql:'SELECT product_id FROM branch_variation_migrations WHERE product_id=? AND branch_id=?',args:[productId,branchId]});if(again)throw Object.assign(new Error('Another operator already established this opening balance'),{status:409});
      ...
      await tx.execute({sql:`INSERT INTO branch_variation_migrations(product_id,branch_id,status,baseline_total,counted_by_employee_id,reason) VALUES(?,?,'counted',?,?,?)`,args:[productId,branchId,total,req.employee?.id||null,reason]});
...
router.post('/reconcile-product/:productId',requirePermission('inventory_edit'),async(req,res)=>{
  try{
    ...
    const {rows:required}=await db.execute({sql:`SELECT b.id,b.name,COALESCE(bi.stock_qty,0) qty,m.status FROM branches b LEFT JOIN branch_inventory bi ON bi.branch_id=b.id AND bi.product_id=? LEFT JOIN branch_variation_migrations m ON m.product_id=? AND m.branch_id=b.id WHERE b.active=1 OR COALESCE(bi.stock_qty,0)<>0`,args:[productId,productId]});
    const incomplete=required.filter(b=>(Math.abs(Number(b.qty||0))>1e-9||b.status)&&!b.status);if(incomplete.length)return res.status(409).json({error:`Variation opening balance is still missing for: ${incomplete.map(x=>x.name).join(', ')}`,control:'branch_variation_migration_incomplete'});
    const {rows:variations}=await db.execute({sql:'SELECT id,name,stock_qty FROM product_variations WHERE product_id=? ORDER BY id',args:[productId]});if(!variations.length)return res.status(409).json({error:'Product has no variations'});
    const tx=await db.transaction('write');let committed=false;
    try{
      const results=[];
