...
function purchasingComposerFlow(type){const t=task(),s=step();if(s!==2)return false;const isPO=type==='po';if((isPO&&t!=='Create, edit, copy, cancel or receive a PO')||(!isPO&&t!=='Create or approve a purchase request'))return false;const workspace=document.getElementById('tt-purchasing-workspace');if(!workspace)return false;const composer=workspace.querySelector('.tt-purch__composer');if(!composer){const launch=workspace.querySelector(isPO?'#tt-purch-new-po':'#tt-purch-new-pr');return guideTarget(launch,isPO?'Start the purchase order':'Start the purchase request',`Tap ${isPO?'New Purchase Order':'New Purchase Request'} to begin. Guide Me will stay with you inside the form.`);}
 const form=composer.querySelector('#tt-purch-compose-form');if(!form)return false;
 if(isPO){
  const supplier=form.querySelector('select[name="supplier_id"]');if(supplier&&!hasValue(supplier))return guideTarget(supplier,'Choose the supplier','Select the supplier or add a new supplier without leaving this purchase order.');
  const branch=form.querySelector('#tt-po-inventory-branch');if(branch&&!hasValue(branch))return guideTarget(branch,'Choose the inventory branch','Choose which inventory branch will own the stock when the goods are received.');
  const ship=form.querySelector('#tt-po-ship-address');if(ship&&!hasValue(ship))return guideTarget(ship,'Confirm the ship-to address','Confirm or edit the physical delivery address. It may be different from the inventory branch location.');
 }
 ...
 const submit=form.querySelector('button[type="submit"]');if(submit)return guideTarget(submit,isPO?'Create the purchase order':'Create the purchase request',isPO?'Review supplier, supplier location, ship-to address, inventory branch, line quantities and cost, then create the PO.':'Review the request details and submit the purchase request.');
 return false;}
function poManageFlow(){if(task()!=='Create, edit, copy, cancel or receive a PO')return false;const s=step(),ws=document.getElementById('tt-purchasing-workspace');if(!ws)return false;if(s===3){const action=ws.querySelector('[data-action="send"],[data-action="approve-po"],[data-action="cancel-po"]');if(action)return guideTarget(action,'Choose the next PO action','Use the appropriate controlled action for this purchase order. Guide Me will progress when the saved change succeeds.');}
