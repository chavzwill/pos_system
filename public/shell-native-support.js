...
function append(domain){const more=document.getElementById('shell-grid'),attention=document.getElementById('shell-attention-grid'),attentionSection=document.getElementById('shell-attention-section');if(!more)return;const items=(registry[domain]||[]).filter(allowed);for(const item of items){const target=item.key==='operations-attention'&&attention?attention:more;if(target.querySelector(`[data-native-support="${item.key}"]`))continue;const card=document.createElement('article');card.className='shell-card shell-card--support'+(item.key==='operations-attention'?' shell-card--attention':'');card.dataset.nativeSupport=item.key;card.innerHTML=`<div><strong>${item.title}</strong><p>${item.desc}</p></div><button type="button">Open</button>`;card.querySelector('button').addEventListener('click',()=>open(item));target.appendChild(card);if(item.key==='operations-attention'&&attentionSection)attentionSection.hidden=false;}}
