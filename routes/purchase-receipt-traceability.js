...
async function ensureReceiptEvidence(){
  ...
  await db.batch([
    ...
    {sql:'CREATE INDEX IF NOT EXISTS idx_purchase_receipts_po ON purchase_receipts(po_id)'},{sql:'CREATE INDEX IF NOT EXISTS idx_purchase_receipt_items_receipt ON purchase_receipt_items(receipt_id)'}
  ],'write');
  // Older PO/receipt schemas did not preserve variation authority. Keep legacy
  // rows NULL so later tracked workflows can fail closed rather than infer.
...
// Explicitly bind a PO line to a catalog variation before physical receiving.
// Once anything has been received, variation provenance is immutable.
router.patch('/:id/items/:itemId/variation',requirePermission('purchasing_receive'),async(req,res)=>{
  try{
    ...
    try{
      ...
      if(Number(item.quantity_received||0)>0)throw Object.assign(new Error('Variation provenance cannot change after receiving has started for this PO line'),{status:409});
      const {rows:[variation]}=await tx.execute({sql:'SELECT id,product_id FROM product_variations WHERE id=? AND product_id=?',args:[variationId,item.product_id]});
      if(!variation)throw Object.assign(new Error('Selected variation does not belong to the PO line product'),{status:409});
      await tx.execute({sql:'UPDATE purchase_order_items SET variation_id=? WHERE id=? AND po_id=? AND COALESCE(quantity_received,0)=0',args:[variationId,itemId,poId]});
...
router.patch('/:id/receive',requirePermission('purchasing_receive'),async(req,res)=>{try{
  ...
  for(const line of requested){
    ...
    validated.push({item,qty,unitCost,lineCost:Number((qty*unitCost).toFixed(2)),identity,line,evaluation,normalized});
  }
  ...
  try{
    ...
    for(const x of validated){
      ...
      const updatedQty=await tx.execute({sql:'UPDATE purchase_order_items SET quantity_received=quantity_received+? WHERE id=? AND po_id=?',args:[qty,item.id,po.id]});if(Number(updatedQty.rowsAffected||0)!==1)throw new Error('Purchase order line changed before receipt commit');
      if(item.product_id){
        if(identity){
