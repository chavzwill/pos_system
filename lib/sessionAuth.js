...
async function sessionAuth(req, res, next) {
  ...
  try {
    const absoluteCutoff = new Date(Date.now() - SESSION_ABSOLUTE_TTL_MS).toISOString();
    const { rows: [session] } = await db.execute({
      sql: "SELECT * FROM sessions WHERE token_hash = ? AND revoked_at IS NULL AND expires_at > datetime('now') AND created_at > ?",
      args: [hashToken(raw), absoluteCutoff],
    });
    if (!session) {
    ...
    const absoluteExpiryMs = new Date(session.created_at).getTime() + SESSION_ABSOLUTE_TTL_MS;
    const idleExpiryMs = Date.now() + SESSION_IDLE_TTL_MS;
    const newExpiry = new Date(Math.min(absoluteExpiryMs, idleExpiryMs)).toISOString();
    db.execute({
      sql: "UPDATE sessions SET last_seen_at = datetime('now'), expires_at = ? WHERE id = ?",
      args: [newExpiry, session.id],
    }).catch(() => {});
  } catch (e) {
    // Downstream authorization remains fail-closed because req.employee is absent.
  }
  next();
}

module.exports = { sessionAuth, createSession, destroySession, destroyEmployeeSessions, setSessionCookie, clearSessionCookie, hashToken, loadEmployee, readCookie, legacySensitiveMutationGuard, SESSION_IDLE_TTL_MS, SESSION_ABSOLUTE_TTL_MS };
