...
const checks=[
 ...
 ['receiving freezes original PO UOM factor instead of re-resolving changed catalog conversion',receipt.includes('SELECT entered_uom,factor_to_base,base_uom FROM uom_usage_snapshots')&&receipt.includes('historical_snapshot:true')&&receipt.includes('factor_to_base:factor')],
 ['explicit receipt in the original PO UOM still uses frozen historical factor',receipt.includes("code(explicitUom)===code(ordered.entered_uom)")],
 ['explicitly different receipt UOM resolves through current allowed purchase conversion',receipt.includes("resolveProductUom(db,Number(poLine.product_id),explicitUom||null,'purchase')")],
 ['historical receipt conversion fails closed if PO base UOM no longer matches product base UOM',receipt.includes('Purchase-order UOM snapshot base unit no longer matches the product base unit')],
