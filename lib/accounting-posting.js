...
async function postSourceJournalWithExecutor({sourceType,sourceId,sourceReference,entryDate,description,branchId,actorId,lines,executor}){
...
async function postSourceJournal(input){
  await ensureLedger();
  if(input.executor&&input.executor!==db){
    return postSourceJournalWithExecutor({...input,executor:input.executor});
  }
  const tx=await db.transaction('write');let committed=false;
  try{
    const result=await postSourceJournalWithExecutor({...input,executor:tx});
