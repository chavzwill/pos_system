...
async function getActiveReservedQty(executor,productId,branchId,excludeReservationKey=null){
  try{
    ...
    let sql=`SELECT COALESCE(SUM(quantity),0) qty FROM inventory_reservations
      WHERE product_id=? AND branch_id=? AND variation_id IS NULL AND status='active' AND expires_at>CURRENT_TIMESTAMP`;
    ...
    // Reservation table is introduced lazily. Availability remains usable before
    // the reservation module is initialized, but once present it is authoritative.
    if(String(e.message||'').toLowerCase().includes('no such table'))return 0;
    throw e;
  }
}
...
async function getAvailableQty(executor, productId, branchId, options={}) {
  ...
  const reserved = await getActiveReservedQty(executor,productId,branchId,options.excludeReservationKey||null);
