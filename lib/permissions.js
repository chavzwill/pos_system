...
function enforceDispatch(req,res){
  const required=dispatchRequirement(req);if(!required)return false;
  if(required==='source_handoff')return false;
  if(req.apiKey){res.status(403).json({error:'API keys cannot operate internal Dispatch workflows',control:'dispatch_rbac'});return true;}
  if(!req.employee){res.status(401).json({error:'Authentication required'});return true;}
  if(required==='source_document'){
    if(['dispatch_view','dispatch_plan','dispatch_execute','dispatch_admin','purchasing','transactions','rentals','work_orders'].some(k=>can(req.employee.permissions,k)))return false;
    res.status(403).json({error:'Missing permission to view this Dispatch source document',control:'dispatch_rbac'});return true;
  }
  if(required==='dispatch_unclassified'){
    res.status(403).json({error:'Dispatch mutation is not classified for RBAC',control:'dispatch_rbac_unclassified'});return true;
  }
  ...
  if(allowed.some(k=>can(req.employee.permissions,k)))return false;
  res.status(403).json({error:`Missing permission: ${required}`,control:'dispatch_rbac'});return true;
}
...
function requirePermission(key) {
  return (req, res, next) => {
    if(enforceDispatch(req,res))return;
    if (req.apiKey) return next();
    if (!req.employee) return res.status(401).json({ error: 'Authentication required' });
...
module.exports = { PERMISSION_TREE, can, requireAuth, requirePermission, requireAnyPermission, dispatchRequirement };
