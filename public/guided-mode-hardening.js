...
function ensureDialog(){const g=guide();if(!g)return;const panel=q('.tt-guide',g);if(!panel)return;panel.setAttribute('role','dialog');panel.setAttribute('aria-modal','false');if(!panel.hasAttribute('aria-labelledby')){const title=q('#tt-guide-title',g);if(title)panel.setAttribute('aria-labelledby','tt-guide-title');}qa('button',g).forEach(b=>{if(!b.type)b.type='button';});}
function safeFocus(el){if(!enabled(el))return false;try{el.focus({preventScroll:true});return true;}catch{return false;}}
function trapFocus(){/* Guide Me is a non-modal coach. Staff must be able to tab into the highlighted workspace control. */}
...
function validateRole(){const p=window.__TT_WORKSPACE_PROFILE__;const g=guide();if(!p||!g)return;const domains=new Set(p.domains||[]);const active=q('.shell-nav [data-domain].is-active')?.dataset.domain;if(active&&domains.size&&!domains.has(active)){announce('This workspace is outside your permitted role. Guide Me will not advance into it.');const next=q('[data-guide-next]',g);if(next){next.disabled=true;next.setAttribute('aria-disabled','true');}}
}
...
window.addEventListener('error',e=>{if(!guide())return;if(String(e.filename||'').includes('guided-mode')){announce('Guide Me recovered from an internal error. Your business action was not submitted automatically.');schedule();}},true);
window.addEventListener('unhandledrejection',e=>{if(!guide())return;const msg=String(e.reason?.message||e.reason||'');if(/guide|guided|workspace|control/i.test(msg)){announce('Guide Me could not complete that navigation step. No business action was submitted.');schedule();}},true);
