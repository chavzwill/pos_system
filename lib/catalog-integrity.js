...
async function catalogHealth({limit=100}={}){
  ...
  const issues=[];
  const {rows:products}=await db.execute({sql:`SELECT p.id,p.sku,p.name,p.active,p.stock_qty,p.category_id,p.brand_id,p.supplier_id,p.online_available,p.image_path,p.is_service,p.is_rental,p.is_non_inventory,
...
module.exports={inspectProductCleanup,archiveProduct,catalogHealth,duplicateProductIssues,normalizeProductName,normalizeSku};
