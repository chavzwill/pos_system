...
async function ensureSchema(){
  ...
  readyPromise=db.batch([
    ...
    {sql:'CREATE INDEX IF NOT EXISTS idx_lifecycle_operation_locks_acquired ON lifecycle_operation_locks(acquired_at)'}
...
async function acquireOne(resourceKey,ttlSeconds){
  ...
  await db.execute({sql:`DELETE FROM lifecycle_operation_locks
    WHERE resource_key=? AND acquired_at < datetime('now','-' || ? || ' seconds')`,args:[resourceKey,ttl]});
  await db.execute({sql:'INSERT OR IGNORE INTO lifecycle_operation_locks(resource_key,token) VALUES(?,?)',args:[resourceKey,token]});
  const {rows:[row]}=await db.execute({sql:'SELECT token,acquired_at FROM lifecycle_operation_locks WHERE resource_key=?',args:[resourceKey]});
  if(!row||row.token!==token)return null;
  return {resourceKey,token};
}
async function releaseOne(lock){
  if(!lock)return;
  try{await db.execute({sql:'DELETE FROM lifecycle_operation_locks WHERE resource_key=? AND token=?',args:[lock.resourceKey,lock.token]});}catch(error){console.error('Unable to release lifecycle lock',lock.resourceKey,error&&error.message);}
}
